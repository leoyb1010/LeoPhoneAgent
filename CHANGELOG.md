...
- 手表直连的模型可以单独选:列出所有能直连的模型(API Key 方式、OpenAI 兼容或 Anthropic 接口的服务商,每家读一次钥匙串),不选就跟默认模型分组走;选的模型被删掉或服务商关掉,回到默认分组。页面上直接写手表会用哪个模型,连不上手表时写明原因(没配对、手表没装 App)。
- 手表直连请求:OpenAI 官方这类新模型(GPT-5、o 系列)用 `max_completion_tokens`,OpenRouter、Mistral 仍用 `max_tokens`(和 iPhone 的 OpenAIProvider 同一规则;原来一律 `max_tokens`,GPT-5 会 400);输出上限从 600 提到 2000,给会思考的模型留出余量,回答长短仍由提示词限制。手表上「总是直连」时 iPhone 在身边也不走 iPhone。
- 验证:Release 真机构建(含手表 App)通过;发版就绪、可见控件、可访问性与动效门禁通过;逻辑测试 405 项通过(改动的文件不在逻辑测试包里,这里只证明没碰坏别的);**未装到 iPhone / 手表上实测**(这次构建时没有连着的真机)。

## Mac 1.2.5 · 安卓、鸿蒙也能批准和全自动,桥接第三轮审计修复 - 2026-09-26

- 用中继钥匙连上的设备(安卓、鸿蒙、主钥匙)和 iPhone 一样能批准审批、开全自动任务、给全自动任务发消息;只有中继认不出身份的老请求仍不能批(e9ba2342)。
- 第三轮审计的桥接修复随这一版发出(5e97b015,明细见 iOS 1.44.0 条目里的「Mac 桌面端桥接」)。
- 状态:Developer ID 签名的 DMG / ZIP 已在本机构建(`~/leophone-mac`);**Apple 公证没做**——这台 Mac Studio 的钥匙串里没有 notarytool 凭据(`leocodebox`),所以还没发布到 `leocodebox-updates`,已安装的 Mac 暂时收不到热更新。

## Harmony 0.3.0-alpha.21 · 流式输出不再截断 - 2026-09-26

### 用户可见

- 本机对话:回答边出边显示到最后。原来经常停在开头几个字或第一段,要退出对话再进来才看全;说完后回答下面的复制、朗读、重新生成也常常不出来。现在说完时列表会跟到底,那一排按钮直接露出来。
- 远控 Mac 的对话:同样不再截断。Mac 中途插进来一条说明(比如「中间有一段记录已经拿不到了」)时,后面的回答另起一段,不再接在那条说明后面。
- 其它列表:改了服务商名字或模型、技能内容、MCP 连上后的工具、定时任务的上次结果、模型组、环境变量的值后,列表马上显示新的(原来要退出页面再进);模型组点「用作默认」立刻变成「默认」;首页对话列表里新起的标题、同组内的顺序,设置里各项的状态文字,都会及时更新。

### 原因

- 鸿蒙的 ForEach 只在一行的 key 变了时才重画这一行。8 月那轮审计为了不在流式输出时每 16ms 重建一次,把聊天列表 key 里的文字长度去掉了 —— 结果正在长的回答一直停在第一次画出来的样子。Markdown 的块、远控对话、另外八个列表是同一个问题(key 只有 id,内容变了不重画)。
- 现在 key 带上这一行显示的全部内容;正在流式输出的那段回答单独用一个状态原地刷新,来几个字只重画这段回答里长了的那一块,列表只在有新消息、工具状态变了这类时候才重排。

### 验证

- 在本机 DevEco 预览器里照 `ChatRun` 的节奏(每 16ms 十个字)回放一段带标题、粗体、列表、代码块的回答:旧代码停在「折叠屏要点」/「展开后宽度到 **6」;新代码在输出中和说完后都完整,说完后复制、朗读、重新生成那一排可见。
- `node protocol.test.mjs`:`PROTOCOL_MACHINES_OK`。新增:在流的回答标 live、说完 / 后面有工具就不是;key 在字变了、同样长的不同字、live → 说完、工具从在跑变没跑完时都要变;聊天列表、远控对话、Markdown 的 key 写法。把 key 改回旧写法,测试当场变红(反向验证过)。
- `build_hap.sh`:未签名 HAP 构建通过。
- **还没装到鸿蒙真机**:需要在 DevEco 里签名后安装,首启应弹出这一版的「本次更新」。

## Harmony 0.3.0-alpha.20 · 界面对齐 iOS / 安卓,折叠屏展开分栏 - 2026-09-26

### 用户可见

- 对话页:回答不再套气泡,每轮上面一行「✦ LeoPhoneAgent」(SOUL.md 里写了名字就用它);你说的话是右边的灰气泡,附件显示在气泡上方。工具调用画成胶囊(按工具换图标,成功绿、失败红、没跑完橙,右边是用时,在跑时转圈,点开看结果);不是最新一轮、至少两步工具的,折成「已工作 N 步 · 用时」,点开展开(iOS 同一规则)。「工具预览」关掉时,说完的都折起来。系统提示夹在两条细线中间;出错是红底一条,最后那条旁边有「继续」;压缩摘要是「已压缩前面 N 条消息」,点开看摘要。说完的回答下面有复制、朗读,对最后一句的回答还有重新生成。往上翻着看时右下角有回到底部按钮,来新消息时变成「新消息」。
- Markdown:行内粗体、斜体、删除线、`代码`(橙字灰底)、链接;标题、有序 / 无序 / 嵌套列表、任务列表、引用、分隔线;代码块黑底绿字,左上角语言,右上角复制(点完变 ✓),太长可滚;表格带边框、按内容估列宽、太宽左右滑。文字可以长按选中复制。原来只有纯文本加灰底代码块,`**粗体**` 这类记号原样露出来。
- 顶栏:标题居中,下面是状态点和模型(点开选模型的底部面板);右边是思考强度(灯泡 + 关 / 低 / 中 / 高,原来「思考中」读起来像「正在思考」)和 ··· 菜单(新对话、压缩前面的对话、导出、朗读)。
- 输入框:一张圆角卡片,上面是模型和审批方式(逐项确认 / 全自动)两个芯片;附件显示成缩略图 / 文件块,可以单独删;输入区随内容长高;下面是 +(照片、文件)、/(命令面板)、话筒、发送,在跑且没打字时发送变成红色停止。写文件审批和「已中断 - 点击「继续」以恢复」都是输入框上方的卡片(和安卓同一句)。
- 首页:顶栏是设置、搜索、远程(电脑图标)、···;下面一排小胶囊(进行中 · N、等你批准、手电筒);对话按置顶 / 今天 / 昨天 / 本周…分组并带数量,每行是按标题配色的头像、标题、最后一句、时间,在跑的有转圈并标「进行中」,置顶的有图钉;长按是带图标的操作面板。手机上最下面是「问 Leo,或直接说要做的事」输入框,发一句就开新对话(照片、文件也能带)。没配服务商时是三步引导。原来的「本机 / 远程」文字页签去掉了,远程在顶栏。
- 折叠屏:展开后宽度够(600vp 起)就左右分栏,不再要求横着拿;展开的折叠屏近似正方形、略高,原来一直是被拉宽的手机版。分栏时左边是对话列表(四成宽,标题「对话」,新对话按钮在顶栏),右边是对话;没选对话时右边提示新建。远程也分栏(机器 | 远程对话),设置有返回。正文和输入框限宽,超宽屏不会一行拉满。
- 设置:浅灰底白卡片,四组(我的设备 / Agent / 外观与通用 / 数据与关于),每行一个实心彩色图标块 + 白色符号,细分隔线;说明文字按现在的功能重写(原来还写着「前台到点执行,杀进程不跑」「HTTP JSON-RPC,没有 Linux 进程」这类已经不对的话)。
- 远程:机器列表是一张卡片(在线 / 离线状态),中继钥匙和配对码收在一张可展开的卡片里;远程对话用和本机一样的气泡、回答头、审批卡片和输入框,会跟着新输出滚到底(原来不会)。
- 其余页面(AI 服务商、技能、MCP、定时任务、记忆、人格、外观……)都有带返回的顶栏;更新记录页改成全部历史(原来只有最新一条,而且是浅灰字在白底上)。图标换成系统符号(和 iOS 的 SF Symbols 一类)。颜色对齐安卓 Theme.kt / ChatColors.kt(照着 iOS 系统色取的),深色模式里卡片上的按钮也看得见了。

### 验证

- 这一版的界面第一次在本机渲染出来核对:用 DevEco 自带的预览器(Previewer)在 Mac 上渲染页面、抓帧截图,手机(1080×2340)和折叠屏展开(2224×2496)、浅色和深色都看过;示例数据是只在预览时塞进内存的,没有进仓库。
- `node protocol.test.mjs`:`PROTOCOL_MACHINES_OK`。新增:Markdown 块和行内解析、工具行解析、聊天条目分组和「已工作 N 步」折叠、用时文字;界面用到的每个图标名都必须在 Symbols.ets 里(写错会悄悄变成「⋯」,这条测试当场抓到过一个);折叠屏按宽度分栏。
- `build_hap.sh`:未签名 HAP 构建通过。
- **还没装到鸿蒙真机**:手势、键盘弹起时的输入框位置、真实字体和滚动手感,要在 DevEco 签名后 `hdc install` 看。

## Harmony 0.3.0-alpha.19 · 本机任务离开页面接着跑、继续、压缩、附件与 App 关着的定时任务 - 2026-09-26

### 用户可见

- 离开页面接着跑:一轮本机任务不再长在聊天页上(`ChatRun`,按对话留在内存里),离开聊天页、切到别的对话都接着跑,列表里标「进行中」。App 退到后台申请短时任务(系统一般给约 3 分钟),快用完时停下、回到前台自动接着跑;在后台说完、出错、等你确认写文件都会发通知(✅ / ❌ / ⏸ / ✋ + 标题,正文是回答开头,和 iOS 同样只在后台发),点通知直接打开那个对话。第一次跑任务时问一次通知权限。
- 继续:一轮开始时在对话档案里记下「在跑」,App 被杀后重开就知道没做完;出错、点了停止、后台暂停也一样,聊天页显示「已中断 · 点「继续」接着做」。继续时最后是你的话就重跑它,否则补一句 iOS 的继续提醒,并把被打断那一轮已经跑过的工具和结果、没跑完的工具(按 iOS 的说法「可能已经执行过,先检查效果」)一起交给模型。停止时说到一半的回答会存下来,继续时模型知道它没说完。
- 压缩:最后一个摘要之后的对话超过 4.8 万字时,发送前先让模型把前面的对话写成摘要(iOS 同一套提示词),最近三轮原样保留;摘要在对话里显示为「已压缩前面 N 条消息」,点开能看;`/compact` 手动压缩。摘要写不出来时照旧只发最近的部分。
- 标题:第一轮说完后让模型起标题(iOS 同一套提示词,最多试 3 次),只替换自动起的标题,改过名的不动。
- 附件与看图:输入框多了「+」,可以附文件(最多 5 个,单个 20MB 内),复制进沙箱,消息里只带 `<user-attached-files>` 清单,模型自己用 `file_read` 读、用新的 `read_image` 看图(缩到最长边 2000、JPEG 85,和 iOS、安卓一样)。`read_image` 的图按各家接口的方式交给模型(Anthropic 放进 tool_result,OpenAI / Gemini / ChatGPT 登录在工具结果后补一条带图的消息)。
- 网页搜索:新工具 `web_search`,先 Bing(国内能直连)再 DuckDuckGo,不用钥匙;另有和 iOS 同一个智谱网页搜索 MCP 预设。
- 技能:提示词里只放技能索引(名字、说明、路径,最多 20 个,和 iOS 同格式),模型用 `file_read` 读 `skills/<id>/SKILL.md` 拿全文;原来每个启用技能的全文每轮都塞进提示词。技能页可以粘贴带 `---` 头的 SKILL.md、点一行改、删除。
- MCP:按规范握手(`initialize` + `notifications/initialized`,协议版本 2025-06-18),带 `Accept: application/json, text/event-stream` 和 `Mcp-Session-Id`,会话过期自动重握手;新工具 `mcp_tools` 列出工具和参数,`mcp_call` 的结果原样给模型;地址和钥匙里可以写 `$$变量名`。修了 `mcp_call` 的参数是对象时被拼成 `[object Object]` 的问题。
- App 关着的定时任务:有开着的定时任务时向系统登记延迟任务,App 关着或在后台时由系统唤起 `ScheduleWork` 补跑今天到点的任务并发通知(系统按使用频率决定多久唤起一次,常用的约两小时,时间不精确)。App 在前台时照旧每 30 秒检查;原来只认「上一次检查到现在」这一分钟,不在前台时到点的就永远错过,现在回来补跑今天这一次。新建或改了时间、而今天的点已过的,从明天开始。
- 远控:Mac 上的 LeoPhoneAgent 任务跟着这台鸿蒙的「全自动」开关走(和 iOS 一样),Mac 不接受时退回逐项审批一次并说明。
- 修复:停下后马上再发,旧一轮等在工具上的那一步会被当成新一轮接着跑(每一轮现在带自己的编号)。

### Mac 端(e9ba2342,等下次发版)

- 用中继钥匙连上的安卓、鸿蒙和主钥匙,和配对过的 iPhone 一样能批准所有操作、能开全自动;只有中继认不出身份的请求还只能拒绝。桥接测试 25 项通过。

### 验证

- `node protocol.test.mjs`:`PROTOCOL_MACHINES_OK`。新增的纯逻辑(压缩、标题、继续、技能索引、MCP 报文、搜索结果解析、定时到点、通知文案)放在 `AgentText.ets`,`protocol/agentText.ts` 是它的逐字镜像并逐项测试;接线处用源码断言检查。
- `build_hap.sh`:未签名 HAP 构建通过,`ScheduleWork` 延迟任务扩展在包里。
- **没有装到任何鸿蒙设备或模拟器上**。后台短时任务、通知点开、延迟任务唤起、附件选择、看图、搜索、MCP 握手都只做了编译和逻辑测试,要在 DevEco 签名后 `hdc install` 实测。

## Harmony 0.3.0-alpha.18 · 本机独立能力与远控对齐 iOS / 安卓 - 2026-09-26

### 用户可见

- 审批两档(和 iOS 同名):「逐项确认」写文件前先问,「全自动」都不问;输入框上方的胶囊、设置、`/auto` 都能切,按设备记住。审批条多了「本次会话允许」和「拒绝并停止任务」,拒绝时给模型的回执与 iOS 同一句话。iOS 的「智能批准」是放行只读命令,鸿蒙没有命令行,所以没有这一档。
- 手机能力交给模型:设备信息、复制到剪贴板、闹钟、定时提醒、通知、手电筒、打开链接或 App、打开拨号盘(不代拨)、定位、天气(Open-Meteo,可按城市或当前位置)。第一次用到时系统弹权限框,除此之外不加确认。
- 离线快捷动作对齐安卓 `ActionRouter`:后天、N 天后、M 月 D 日、(下)周几;出行在出发时和出发前一小时各提醒一次;带时间的待办变成提醒;缺时间或车次时先问,不让模型猜;复制和设备信息也走本机。
- 本机 Agent:同一工具同一参数连续重复先提醒、再停(`ToolLoopGuard`),单次最多 50 轮工具;Anthropic 流中途断开按失败处理并换线路;模型报错改成人话;工具参数不完整不执行;停止或换对话后迟到的回调不再写进别的对话。
- 长对话:原来每轮都把整段历史连同每张旧图重发,聊久了必然超限报错。现在按 6 万字的上限从最早的整轮丢起(最新一句总会发),图片只发最近两张,更早的换成一句说明,并告诉模型前面省略了。
- 环境变量的值不再发给模型,只给名字,执行工具时再替换 `$$NAME`;删服务商时一并清掉模型分组里它的条目;ChatGPT 登录可拉官方 Codex 模型目录;模型目录补上 GPT-6 / GPT-5.6、Claude Opus 5、Gemini 3.1 Pro、Grok 4.6。
- 会话菜单「导出对话」(复制为 JSON);启动页选「自动」时打开 24 小时内的上一个对话。措辞统一为「AI 服务商」。
- 远控 Mac / 手机:打开时不再建空任务,第一句话才建;打开时可挑没结束的任务接着看,或接手 Mac 桌面上的任务;审批按钮按 Mac 给的选项(允许一次 / 本次会话允许 / 始终允许 / 拒绝)再加「拒绝并停止任务」;只有 401 才说钥匙不对,其他错误显示中继或 Mac 的原话;每个 POST 带 `X-Leo-Request-Id`,读超时 65 秒;按 seq 去重,续传缺段会说明;一轮结束后流正常关闭不再重连;平板两栏在同一台机器上换 Agent 或任务会重新接上;Mac 发的配对码加入时等 Mac 同意(202 + 轮询)。
- 被远控时(鸿蒙当身体):流保活每 25 秒(与安卓一致);已答过的 POST 按请求编号直接回放;关闭码 4001 / 4003 说清原因,4003 不再重试;续传用信封代替 410;非流式回答以 `run.failed` 收尾;`run.completed` 带实际吐出的文字,换线路和工具轮不再重发已发的字。

### 验证

- `node protocol.test.mjs`(DevEco 自带 node,在 `src/harmony/protocol` 下跑):`PROTOCOL_MACHINES_OK`。新增断言对旧代码反向跑过会红;`protocol/actionRouter.ts`、`harnessTypes.ts` 里的镜像与 ETS 一字不差(改一边会红)。
- `src/harmony/scripts/build_hap.sh`:未签名 HAP 构建通过,闸门 `verify_harmony_release_notes.sh` 在链首。
- **没有装到任何鸿蒙设备或模拟器上**:手机能力(定位、天气、闹钟、拨号盘等)和远控改动只做了编译和逻辑测试,要在 DevEco 签名后 `hdc install` 实测。

### 没做

- 用中继钥匙直连 Mac 时,Mac 把鸿蒙当旧版客户端:只能批准只读工具,其余返回 403(鸿蒙会显示原因);用 Mac 发的配对码加入就没有这个限制。
- 聊天页关掉后 Agent 回合不在后台继续、工具行不落盘续跑、像 iOS 那样先让模型写摘要再压缩(现在是直接丢最早的)、附件读图、MCP、技能索引、App 关闭时的定时任务、网页搜索、AI 起标题:下一批。

## iOS 1.44.0 (126) · 审批三档、iPad 检查器与多窗口、步骤折叠、iOS 27 - 2026-09-25

- 审批:输入框上方的开关改为三档菜单(逐项确认 / 智能批准 / 全自动),设置页同一组选项,`/auto` 仍切全自动。智能批准按确定性规则表(`CommandRisk`)放行只读命令和不碰个人数据的只读手机能力(天气、地图、设备信息、文本分析等);风险等级只用于提示(弹窗标题、手表颜色),不增加确认步骤;同一会话允许过的不再重问。凭证审批改用统一措辞并新增「拒绝并停止任务」;本机敏感操作和 Mac 任务的审批通知带「批准一次 / 本次会话允许 / 拒绝 / 拒绝并停止任务」,批准不再要求先解锁;手机能力审批新增「始终允许」(改为不再询问,设置 → 权限可改回);`SensitiveToolGate` 的等待阶段改由 App 注入的钩子上报,逻辑测试包不再引用聊天栈。
- iPad:`.commands` 菜单栏与硬件键盘快捷键(⌘N、⌘,、⌘F、⌘K、⌘1–9、⌘.、⌥⌘I);聊天检查器(运行 / 会话 / 产物 / 文件),竖屏系统把检查器浮在内容上时按实际遮挡量让出安全区,输入栏不再被盖住;会话右键「在新窗口打开」、拖出会话行开新窗口(`NSUserActivity` + `UISceneSessionActivationRequest`;启动时显示哪一页仍按「启动页」设置)、iPadOS 27 `UIWindowScene.closureConfirmation` 关窗确认(可「停止任务并关闭」);宽面板中的设置改为分组侧栏 + 详情两栏(默认打开 AI 服务商);网页链接和纯文本拖入输入框,回复中的图片可拖出(带原文件);可读宽度 800 pt;检查器里的工具详情在 iPadOS 27 停靠右侧。
- 消息列表:完成的一轮(非最后一条回复,且至少两次工具调用)把最终回答之前的步骤折叠为「已工作 N 步 · 用时」,点按展开或收起,展开不触发「新消息」胶囊;思考块显示「思考了 N 秒」;工具条出现导致底部留白增大时不再重新打开自动跟随;流式解析不再为每个文字/思考增量跳主线程(思考文本离主线程累积、节流时同步,取消和出错时补齐);流式中的表格尾巴恢复为纯文本(原先的「已定稿」判断在流式期间恒为真);逐词淡入只作用于正在流式的文本,跳过等宽代码与附件,「减弱动态效果」或 iOS 27 省资源时关闭。
- 输入栏:发送 / 排队 / 停止合成一个身份稳定、图标形变的按钮,⌘↩ 只发送;停止时长按菜单可清空队列;输入框补上无障碍名称;iOS 27 导航栏随下滑收起;反馈面板有未发送内容时关闭前确认(iOS 27)。
- 灵动岛 / 实时活动:更新串行执行,运行中状态带 5 分钟过期时间并每 2 分钟心跳续期,过期显示「可能已中断」;等待批准的会话显示橙色并以提醒更新;`.supplementalActivityFamilies([.small])` 给手表 Smart Stack 与 CarPlay 专用布局;iOS 27 灵动岛变窄时只显示数字;隐私模式切换立即重推;Mac 任务结束按真实结局(完成 / 停止 / 失败 / 离开页面)记录。
- 通知:统一注册所有可操作类别(修复各处以空按钮覆盖类别的问题);任务完成与快捷指令通知支持直接回复(进行中则排队);时效性通知权限;正在查看的会话不弹横幅。
- Apple Watch:朗读只在抬腕时自动进行、放下暂停、最多 4 句、结束以 `.notifyOthersOnDeactivation` 交还音频(watchOS 27 异步停用),可关自动朗读;直连请求在网络中断或抬腕中途放下时转后台 URLSession,答案用通知送达;审批卡按风险着色、双指互点即「允许一次」;「问 Leo」在 App 已在前台或从后台回来时也会开始听;显示 iPhone 上正在运行的任务;watchOS 27 省资源时停动画;修复取消提问在 iPhone 上收不到的问题。
- iOS 27 与系统:`BGTaskScheduler.submitTaskRequest` 异步提交;快捷指令「等待结果」采用 `LongRunningIntent.performBackgroundTask`;`isVoiceOnly` 时要求并朗读短回答;布局上限改按当前窗口而非整块屏幕(去掉全部 `UIScreen.main`)。FoundationModels 自定义模型与私有云模型的技术验证结论见 `docs/upgrade-2026-09-25/foundationmodels-spike.md`(私有云模型需向 Apple 申请受管权限,本版未接入)。
- 其他修复:聊天浏览器面板接管后关闭即交还 Agent(原先一直等待);浏览器提示词如实说明起始 User-Agent;Mac 任务的后续消息带 `X-Leo-Queue` / `X-Leo-Request-Id`,Mac 离线时由中继排队。
- Mac 中继(未部署,需在 cortex 更新后生效):`message.delta` 每 60 ms 合并一条且不改变顺序与续传游标;慢速手机改为整队清空后关流并续传,不再留下 seq 空洞;大段回放时让出事件循环,避免健康的手机被当成慢消费者;`run.failed` 不再推送提醒。
- 验证:404 项逻辑测试通过(新增 `CommandRiskTests`);新增可访问性审计 UI 测试(首页、新对话、设置:控件名称、可识别性、特性),去掉输入框名称时会红(已反向验证);端到端用本机 OpenAI 兼容假模型服务 + 调试版 JSON-RPC 驱动真实流式链路,在 iOS 27 iPhone 18 Pro 模拟器上截到思考用时、逐词淡入、流式表格尾巴、两次工具调用、第二轮后自动折叠与展开收起、「更多」菜单扩大后的点按;iOS 27 iPad Pro 13 寸模拟器用 Release 构建 + 1 MB 主线程栈(与真机一致)巡检首页、⌘K / ⌘F、检查器竖屏让位、设置两栏,无崩溃;watchOS 27 Series 12 模拟器安装启动;中继 28 项测试通过;Android 两个 flavor 各 643 项单测通过、lint 0 错误。未在 iPhone、iPad、手表真机上安装验证。
- 发版前三轮审计(代码审计 + iOS 26.5 / 27 模拟器实跑,版本号不变):
  - 第一轮:锁屏「本次会话允许」发给 Mac 上的 Claude Code / Codex / Grok 任务不再直接被拒(400);智能批准堵上多行脚本、`$( )`、反引号、`<( )`、`command`、`sed -i`、`find -fprint`、`sort -o`、`git --output`;远程主机的会话授权按主机生效;快捷回复不再误报、抢会话或带走草稿;Mac 会话的实时活动保持「等你处理」;消息列表折叠、表格尾巴、测宽修正;iPad ⌘. / ⌥⌘I 作用于当前窗口,「在新窗口打开」打开对应会话;⌘K 的 Esc、回车、硬件键盘;手表转后台的提问不重复发送。
  - 第二轮:Mac 追问发送失败会说明并放回输入框,断开后下一条消息重新跟进;断流 / 输出上限给「继续」;审批弹框改为 UIKit 顶层弹框,不再关闭已开的设置或控制台;iPad 横屏键盘弹出时按尺寸类别而非实测高度判断分栏;横屏 iPhone 输入栏精简;⌘K 显示真实会话名;中文文案统一(快捷任务、AI 服务商、允许一次),补 23 条本地化。
  - 第三轮(故障下的健壮性):排队追问注入新一轮后流式输出按新气泡的 id 写入;循环内自动压缩不再清掉 `isProcessing`、不再另起一个循环,压缩后按 id 重新定位当前气泡;助手这一步在工具执行前落库(原先整批工具返回后才写),App 中途被杀后「继续」用「可能已执行」的占位结果接着做,未作答的提问从该提问重跑;对已保存的回答点重试会同时删掉旧行和旧文字,保持「历史条目 = 数据库行」的截断约定;`minis.db` 加 5 秒忙等,合同版本已是最新时迁移不再开写事务、不再整表回填;失败的回合不再把错误记到上一条好回答上;后台时间到期只结束命令、不算用户取消,回到前台接着跑;删除对话时丢弃排队提示,待推送的删除不再被迟到的写入改回更新;iCloud 强制拉取跟随查询游标读完,运行中的对话拒绝拉取;Mac 控制台把「已取消」当作一轮结束而不是停止跟进,中继排队的后续消息回查送达结果(被拒 / 失败时放回输入框);Mac 审批在一轮结束时清掉、409 视为已处理、刷新不再读到旧的会话表;锁屏「本次会话允许」只回退到「允许一次」;远程主机只在认证失败时改用设备钥匙且不删密码;Siri 停止 Mac 任务优先停正在跑的;工具参数被截断时不再补全后执行(预检拒绝、模型重发,iOS / Android 同改)。
  - 验证:逻辑测试 405 项通过(新增迁移不占写锁、截断参数不补全两项,均已反向验证);Python 中继与 harness 35 项、Mac 桥接 TypeScript 25 项、Android 两个 flavor 各 648 项通过,新增用例均在旧代码上会红;发版就绪、可访问性与动效、可见控件、原生权限四道门禁通过;手表构建通过;iOS 27 iPhone 18 Pro 模拟器用假模型实跑两次工具调用与审批流程,落库行数与顺序正确,Release 构建 + 1 MB 主线程栈打开完整会话无崩溃。重试修复、后台到期、Mac 控制台与中继排队回查只做了代码核对和单元测试,未在真机或真实 Mac 上复现。
- Mac 中继与 leoagent(未部署):同名重连时旧连接的流和请求会结束;排队消息取出即标为「已排队」,换连接或连接出错时放回队列并交给新连接,已吊销设备的排队请求不再送达;Claude / Codex / Grok 支持「本次会话允许」并按各自协议映射(Claude 只写会话级规则);CLI 退出或停止时关闭待批审批,已结束会话的审批返回 409;未登录的 Codex / Grok 首条消息报失败而不是一直转圈;达到 16 个会话上限时先停掉一个空闲会话;Python 路径的事件流每 25 秒发保活。需要在 cortex 更新中继、在本机更新 `~/.leoagent` 后生效。
- Mac 桌面端桥接(未发版,随下一版 Mac 发布):一轮结束时给仍挂着的审批发拒绝回执;发送失败写 `run.failed`,不再一直「运行中」;只有手机发起的一轮才代答提问和推送;停止与正常结束撞上时不再补一个「已取消」;中继不认存着的机器钥匙(4001)时改用注册钥匙重新领取;`leo-link-switch.sh --rollback` 回滚前先解除机器名钉扎。
- Android(未发版,随下一个 APK 发布):系统文字选择菜单「问 LeoPhoneAgent」(`ACTION_PROCESS_TEXT`,预填输入框,超长转附件);Power 版无障碍操作前在目标上画编号高亮(无障碍浮层,不进界面树与截图);删除未用的 Markdown 导入与 `BorderedMarkdownTable`;审计修复:截断的工具参数不再补全后执行;助手这一步在工具执行前落库;定时任务不再占着闹钟广播跑完整轮,超时不发「完成」通知,解析不到会话时不启动前台服务;发送后立即停止真的会停;Anthropic 流中途的 `error` 事件和没到结尾就断流会重试,不再当成完整回答;远程回合不设 10 分钟上限,追问不再把上一轮回答当作第一段增量重发;本机被控时未知会话在流内报 `run.failed`、空闲时每 25 秒保活;系统粘性重启前台服务后立即退出;遥控页只显示对方仍在等的审批,409 视为已处理,403 显示对方给的原因;事件流缓冲不再丢事件。

## iOS 1.43.0 (125) · 后台状态、模型收藏、iPad 首页与手表直连 - 2026-09-25

- 灵动岛与后台：运行中的任务不再因工具报错显示「失败」；持续处理任务的进度每个事件都前进，系统不再把长任务判为停滞而报失败，到期时如实标为仍在运行或已暂停；等待回到 App 的任务不再被记成完成；静止状态按真实结局显示（完成绿色、失败或待处理橙色、手动停止灰色），旧版 Live Activity 数据兼容解码。
- 模型收藏：删除服务商、模型或分组时级联清理收藏、最近使用、默认分组指针、Agent 可调用分组和压缩/标题模型槽；收藏上限只计可见条目，历史遗留的失效收藏在下次收藏时清掉；删除会话同时清掉它的模型绑定和推理配置；模型胶囊显示实际会用的模型。
- iPad：未选会话时右栏是首页工作台（与 iPhone 首页同一输入栏），修复 iPad 上「/」面板、亮度选择和 Mac 控制台入口无响应；设置、藏宝阁、Mac 控制台改为整页面板；新对话起手式不再被输入框遮住（iPhone 同样受益）。
- Apple Watch：只保留说话、看答案、朗读、追问与记录；iPhone 不在身边时由默认分组里可直连的模型直接回答（设置 → Siri 指挥中心 → Apple Watch，可关闭，Key 仅存手表钥匙串）；息屏常亮时停止循环动画。
- 稳定性：启动时不再在主线程同步等待定位服务（模拟器中定位服务卡死时，旧版会停在启动页）；删除约 1500 行无人调用的代码。
- 验证：400 项逻辑测试通过（新增进度单调、旧快照解码、收藏键清理、模型槽清理 4 项）；iPad Pro 13 寸模拟器竖屏、横屏截图巡检；手表模拟器直连 DeepSeek 实测回答；真机目标构建通过。未在 iPhone、iPad、手表真机上安装验证。
- Android（未发版，随下一个 APK 发布）：删除服务商时只删除被清空的分组并同步清理默认分组、语音分组和 Agent 可调用分组指针，同时清掉该服务商的 OAuth 令牌。

## iOS 1.36.0 (111) · 系统转写与录音生命周期 - 2026-09-12

- SpeechAnalyzer / SpeechTranscriber 本机转写与主动语言资源管理；转写请求不自行下载资源，缺失或不支持时明确降级。
- 自动／离线选择不再被分组 Online 成员覆盖，新默认输入组使用 Auto，已有用户配置保留。
- 权限等待请求有独立身份，关闭、停止、后台和编辑切换可取消；系统不允许恢复时停止，音频路由恢复在主线程执行。
- 语音执行位置及部分结果可见；iOS 远程控制台显示 W04 日志持久化状态。
- 396 项逻辑测试、发版/原生权限/设备动作/录音中断/模式选择门禁与签名构建通过；iPhone 17 Pro Max 已经通过 Wi-Fi 从 1.35.0 (110) 覆盖安装至 1.36.0 (111)，版本回读及启动请求成功。真机截图因开发连接中断未取得，语言资源实际转写与通话/耳机组合仍待完整验收。详见 [装机记录](docs/upgrade-2026-09-12/IPHONE_1.36.0_DELIVERY.md)。
- 修复 Release 生成脚本无条件重写相同 Swift 文件的问题，避免后续无变化构建再次触发整模块编译；实际脚本的内容和修改时间回归通过。

## Mac 1.85.0 · 队列、日志与工作台交互 - 2026-09-12

- 持久化任务队列、跨轮游标、多观察者与审批去重；首页草稿、项目选择、全局查找、设置归属和模态焦点调整。
- 远程日志保存状态可见，关键推送等待落盘，收据校验持久性；回放、摘要读取与启动日志索引改为异步有界处理，慢连接遵循反压。
- 远程控制台稳定行标识、400 行展示边界、上滚阅读保护与重复发送保护。
- 通知设置写入校验实际主 frame；multer 2.3.0、hono 4.13.5、js-yaml 3.15.2 / 4.3.2、sharp 0.35.4，完整 npm 审计 0。
- 已发布至主仓库与 leocodebox-updates：桌面 45/45、前端 194/194、服务端 446/446，类型检查、lint、更新记录及构建通过；36 个嵌套 Mach-O 签名通过，App/DMG Apple 公证与钉章、下载隔离副本 Gatekeeper 通过。两个仓库的三个附件摘要一致，真实 Electron 更新器确认公开源可发现 1.84.0 → 1.85.0。详见 [发布记录](docs/upgrade-2026-09-12/MAC_1.85.0_RELEASE.md)。
- 完整 W01–W23 尚未全部完成，精确窗口原生交互仍保留未验收状态。

## iOS 1.35.0 (110) · 本机能力与任务回执 - 2026-09-12

- 首页基础系统动作无需模型配置；统一本机授权入口，手电筒与亮度回读真实状态，相机取消和超时释放占用。
- 任务、快捷指令与组件绑定具体运行及结果消息，区分失败、取消、等待和未确认，避免串用其他运行结果。
- 系统离线语音不再静默退回联网识别。
- 390 项逻辑测试通过，Release 构建及签名校验通过；iPhone 17 Pro Max 已通过 Wi-Fi 从 1.34.0 (109) 覆盖安装，回读 1.35.0 (110)，App 启动及主程序、Widget 进程核对通过，截图确认首页和能力中心。详见 [交付记录](docs/upgrade-2026-09-12/IPHONE_1.35.0_DELIVERY.md)。
- 完整升级计划仍在进行；SpeechAnalyzer、Mac 真窗口验收等尚未完成，未宣称全量交付。
