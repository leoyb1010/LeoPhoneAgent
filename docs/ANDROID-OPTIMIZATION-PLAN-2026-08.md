...
## 2026-08-22 落地状态（alpha.12）

本节是当前事实来源；下文保留原始审计上下文，不能再把已经完成的条目当成未实现。

| 原计划 | alpha.12 状态 | 当前实现 |
|---|---|---|
| CLI 进入聊天模型选择器 | 已落地 | 四家 CLI 安装探测、版本、未安装跳管理页、`cli` 会话绑定 |
| CLI 非终端流式对话 | 已落地第一版 | `CliChatEngine` + 四家 JSONL 方言 + 同 cwd 续聊 + 排队消息 |
| API Key 安全桥接 | 已落地 | Prompt 私有临时文件；Key 瞬时环境注入，单轮结束恢复/清除 |
| CLI 工具审批映射到聊天审批卡 | 未完成 | 当前保持各 CLI 的安全默认；不自动绕过审批，不虚报完成 |
| Artifact 卡片 | 已落地 | HTML/Markdown/SVG/CSV/PDF/图片/音视频写入后直接预览 |
| Relay SSE + 断线续传 | 已落地 | `after=seq` 回放、实时跟随、审批、终态、指数退避 |
| 远程会话并入本机会话列表 | 未完成 | 仍在「远程机器」工作台实时显示，尚未伪装成本地 Room 会话 |
| Relay 文件互传 / 剪贴板 / 扫码相机 | 未完成 | 配对文本已存在；文件协议和扫码权限仍需跨端设计 |
| 图片生成 | 已有底座 | `minis-model-use` 与行内媒体可用；独立 Provider 入口继续后置 |
| Skill 同步 / 本机 OpenAI 网关 / 手机 MCP | 探索项 | 不纳入 alpha.12 发布承诺 |

alpha.12 的发布定义只覆盖“已落地”行。需要新协议、系统权限或跨端迁移的条目继续保留，
不会为了写成 100% 而用假按钮、危险自动批准或仅文档占位替代真实能力。
