...
状态:**1.44.0 (126) 已落地大部分,并做完发版前三轮审计**(见下方「落地状态」「发版前审计」)。每一项都写了参考来源、做法、代价和验收办法;按阶段发版,每一版照发版铁律 bump 版本并写「本次更新」。

## 落地状态(2026-09-25,iOS 1.44.0)

用户在施工中追加的原则:**个人自用,以方便为先,不加安全流程;保留全自动。** 因此计划里的「高风险转表冠确认」「安全选项排第一」「高风险不给会话授权」都改成只做风险提示,不加步骤。

已做:审批三档(智能批准 + `CommandRisk` 规则表 + 手机能力的非个人只读放行)、通知里批准 / 拒绝并停止 / 始终允许、通知直接回复;iPad 菜单栏、检查器(含竖屏浮层让位)、多窗口(右键 / 拖出会话开新窗口)+ 关窗确认(按窗口恢复会话没做:会绕过用户的「启动页」设置)、设置两栏、拖入链接文字、拖出图片、可读宽度、`UIScreen.main` 清零;消息列表「已工作 N 步」折叠、思考时长、跟随策略修正、流式主线程跳转削减、表格尾巴修复、淡入范围收窄;形变发送键;灵动岛过期 / 心跳 / 串行更新 / 等待批准提醒 / `.small` / 窄岛 / 隐私重推;手表朗读礼仪、后台续答、风险着色审批、运行状态;iOS 27 的省资源、异步后台提交、`LongRunningIntent`、`isVoiceOnly`、导航栏收起、关闭确认、侧边停靠;可访问性审计 UI 测试;中继 60 ms 合并(未部署);Android 文字选择入口与操作浮层(未发版)。验证用本机假模型服务 + 调试版 JSON-RPC 驱动真实流式链路。

发版前审计(2026-09-25 ~ 26,版本号不变,修复并入 1.44.0 的「本次更新」和 CHANGELOG):三轮,每轮换一个角度,每轮都是审计 → 测试 → 修复 → 提交。
- 第一轮 代码审计 + iOS 26.5 实跑:审批(锁屏会话授权、智能批准漏放的写法、远程主机授权范围)、通知快捷回复、Mac 会话实时活动、消息列表折叠与表格、iPad 多窗口与 ⌘K、手表后台提问;中继同名重连不结束旧流(已修,未部署)。
- 第二轮 用户流程 / 一致性 / 本地化:Mac 追问失败回填、断流给「继续」、审批弹框不关已开页面、iPad 横屏键盘不塌栏、横屏 iPhone 输入栏、⌘K 真实会话名、中文文案统一与 23 条缺失翻译。
- 第三轮 故障下的健壮性(断网、中继离线、前后台、取消、持久化)+ Mac 与 Android 审查:iOS 15 项(排队追问写错气泡、循环内压缩打断运行、工具前落库、重试留旧行、SQLITE_BUSY、错误记到上一条、Mac 控制台取消后不再跟、Mac 审批残留、后台到期当成取消、锁屏会话授权升级成始终、SSH 密码被删、强制拉取只读一页、删除对话复活、Siri 停错任务、灵动岛不反映控制台)全部修复(强制拉取仍按「以云端为准」整体替换,不保留还没推上去的本地行);Mac 端 12 项修 11 项(非 iPhone 设备可批 Claude/Codex/Grok 一项按「自用、方便优先」不收紧);Android 12 项都已修复(遥控页关掉后首页的待审批数不清零这一小点未改,页内已只显示对方仍在等的审批);iOS / Android 都去掉了「截断参数补全后执行」。
- 仍待真机或真实环境确认:重试修复、后台到期续跑、Mac 控制台与中继排队回查只做了代码核对和单元测试;中继(cortex)、本机 `~/.leoagent` 的 Python 副本、Mac 桌面端、Android APK 的修复都要各自部署或发版后才生效。

没做或改了做法(原因):
- 逐帧抖动 UI 测试:它依赖的测试桩 8 月起就已删除,测试一直跑不起来,本版删掉;改用上面的假模型 + 调试 RPC 端到端巡检。
- 首页提醒条 / 会话行显示「需要审批」:本机审批是模态弹窗,会立刻出现在最前面;Mac 审批提醒条已有。
- `AVAudioSession` 新中断通知与异步激活、`withTaskCancellationShield`、MetricKit 新 API:旧接口在 27 仍可用,迁移需要真机来电 / Siri 打断实测,留到部署目标升到 27 时。
- 浏览器 `@eN` 引用、截图按需、不可信边界、域名白名单、钥匙串代填、`render_ui`、跨会话回忆:改动面大,本版只修了接管后不恢复和 UA 说法两个实际问题。
- 终端底部面板、活动查看器、LeoMotion 全量收口、`@ScaledMetric`:App 有自己的字号设置,审计确认动态字体告警属设计如此。
- 中继按在场免推送:需要改 `apns.py`;手机端先做了「正在看的会话不弹横幅」。
- 手表控制中心「问 Leo」控件:需要新建手表组件扩展 target,本版未加。
- FoundationModels:结论见 `foundationmodels-spike.md`(私有云模型需受管权限)。
