...
final class AIChatViewModel: ObservableObject, SpeechControlling {
    ...
    init() {
        ...
        syncChangeObserver = NotificationCenter.default.addObserver(
            ...
            // "LeoPhoneAgent is thinking" indicator. Flip a flag instead; the
    ...
    /// [T-auto-compact-live] 每次读都去问 UserDefaults,**不再**在 VM init 时
    /// 快照一次。原来是 `@Published var ... = UserDefaults...bool(...)`:VM 被
    /// ViewModelCache 缓存并跨设置页存活,用户在「外观 → 压缩 → 到阈值自动压缩」
    /// 里翻开关(那边是 @AppStorage,直接写 UserDefaults),已经打开的会话到下次
    /// 冷启动前都还按老值走,开关看起来没生效。
    /// 写入这一侧仍然回写 UserDefaults,并手动发一次 objectWillChange,让绑到
    /// 它的界面照常刷新。
    var autoCompactEnabled: Bool {
        get { UserDefaults.standard.bool(forKey: Self.autoCompactDefaultsKey) }
        set {
            guard newValue != autoCompactEnabled else { return }
            objectWillChange.send()
            UserDefaults.standard.set(newValue, forKey: Self.autoCompactDefaultsKey)
        }
    }
    static let autoCompactDefaultsKey = "autoCompactOnThreshold"
    ...
    /// LeoPhoneAgent-2026-06-01-134710.ips: user switched sessions while both vms had
    ...
    /// Matches: ![...](leophoneagent://attachments/file.mp3?auto_play=true)
    ...
    static func triggerAutoPlayAudio(in text: String) {
        ...
        // Resolve leophoneagent:// to file URL
        let fileURL: URL?
        if cleanURL.scheme == "leophoneagent" {
    ...
    /// Resolve leophoneagent:// URL to local file URL (standalone, no UI dependency).
    ...
    private var baseSystemPrompt: String {
        ...
        SystemPromptBuilder.identitySection()
            ...
            // LeoPhoneAgent has no in-app scheduler (see 'Scheduled tasks'), so the only
            ...
            + "IMPORTANT: leophoneagent:// URLs are app-internal — they are NOT web URLs. Do NOT pass leophoneagent:// action URLs (open_terminal, views, settings) to browser_use — those are app deep links, use Markdown links in chat instead. "
            + "However, leophoneagent:// resource URLs CAN be opened in browser_use with navigate. All directories under /var/minis/ are accessible: workspace, attachments, offloads, shared, etc. "
            + "The built-in browser fully supports leophoneagent:// — HTML pages and all sub-resources (JS, CSS, images, fonts, etc.) referenced via leophoneagent:// absolute URLs or relative paths resolve correctly within the current session. "
            ...
            + "The browser resolves relative paths against the leophoneagent:// base URL automatically. "
            + "Cross-directory references also work with absolute leophoneagent:// URLs (e.g. <img src=\"leophoneagent://attachments/photo.png\"> from a workspace HTML page). "
            + "Navigate to the entry HTML to preview, e.g. leophoneagent://workspace/myapp/index.html.\n"
            ...
            + "IMPORTANT: leophoneagent:// URLs MUST be percent-encoded. Non-ASCII characters (Chinese, emoji, spaces, etc.) in filenames will break Markdown rendering if not encoded. "
            + "Use the minis_url from tool results directly — it is already encoded. "
            + "If you construct a leophoneagent:// URL manually, percent-encode the filename (e.g. %E4%B8%AD%E6%96%87 for non-ASCII characters).\n"
            ...
            + "Audio auto-play: append ?auto_play=true to an audio leophoneagent:// URL to auto-play when rendered (e.g. ![audio](leophoneagent://attachments/song.mp3?auto_play=true)). Use sparingly — only when the user explicitly asks to hear audio immediately.\n"
            ...
            + "Use Markdown links for all leophoneagent:// files — the user can tap to preview them directly in chat.\n\n"
            ...
            + "- Route in this order: Apple framework/App Intent → Shortcut or URL scheme → user-visible app handoff. iOS does not grant arbitrary cross-app Accessibility control; do not pretend it does.\n"
            + "- A successful command is not sufficient proof. Read back the created system object or verify the resulting state when the framework supports it. If verification is unavailable, say so explicitly; never claim success from a guessed state.\n\n"
            + "Tone and style:\n"
            ...
            + "apple-alarm sets alarms and timers via AlarmKit (iOS 26+). Alarms can only be viewed in the LeoPhoneAgent home screen (alarm icon in the top-right toolbar) or by opening leophoneagent://views/alarm. "
            + "After setting an alarm, tell the user it is visible on the LeoPhoneAgent home screen and they can tap the alarm icon or open leophoneagent://views/alarm to manage it.\n"
            ...
            + "apple-contacts queries and manages the user's contacts: list, search --query (name/phone/email/org), get --id, groups; create/update/delete REQUIRE --confirm — ask the user before any write. To actually call or message someone, look up the number here then use apple-open tel:/sms:.\n"
            + "apple-files manages external folder access: list shows folders the user has granted (mounted at /var/minis/mounts/<name>/ for shell access), request opens a system folder picker for the user to grant a new folder (--name to choose the mount name, --read-only for no-write), pick opens a file picker and copies chosen files into /var/minis/offloads/, reauth --name re-resolves a stale grant, remove --name --confirm revokes one. Pickers require the app in the foreground and the user completes the selection.\n"
            + "apple-camera captures through a user-operated camera UI: photo (user takes a picture, --camera front|back), scan-code (QR/barcode scanner, returns payload), scan-document (multi-page document scanner). Files land in /var/minis/offloads/ — chain with apple-vision ocr to extract text. Requires foreground; never captures silently.\n"
            + "apple-shortcuts runs the user's Shortcuts via x-callback-url: run --name <exact name> [--input <text>] switches to the Shortcuts app and runs it (the user sees everything; control returns here after). iOS cannot enumerate the shortcuts library — use register --name --desc / list to maintain a registry, or ask the user for exact names. This unlocks Apple Notes, settings toggles, and any third-party app actions the user has wired into shortcuts.\n"
            + "apple-motion reads live motion data: steps [--days 1-7] (per-day pedometer from the coprocessor — more current than HealthKit for today), activity [--days N] (walking/running/driving/stationary segments), status. Only 7 days of history; older data via apple-healthkit.\n"
            ...
            + "Interactive terminal: leophoneagent://open_terminal opens a terminal for tasks that require interactive stdin (passwords, ssh, TUI apps like htop/vi). "
            ...
            + "Examples: [Open Terminal](leophoneagent://open_terminal), [Login to SSH](leophoneagent://open_terminal?init_command=ssh%20user%40host).\n\n"
            ...
            + "[Set ENV_NAME](leophoneagent://settings/environments?create_key=ENV_NAME&create_value=&create_note=Used%20by%20XYZ) — "
    ...
    func retry() {
        ...
        // [T-retry-target] 这里刻意保留 `messages.last`,不跟 resume() 一起换成
        // lastAssistantIndex。retry 会就地重生成这条 assistant 的内容;当队列形态
        // 是 […, assistant, user(排队中)] 时,用 lastAssistantIndex 会去重写那条
        // 更早的 assistant,新文本就出现在用户刚发的消息**上方**,时间线错乱。
        // resume() 不同:它是"把中断的循环接着跑完",目标本来就该是最后一条
        // assistant,所以那边用新规则是对的。
        guard let lastMsg = messages.last, lastMsg.role == .assistant else { return }
        let candidateIdx = messages.count - 1
    ...
    private func handleUserCancelledCleanup() {
        ...
        if userDidCancel {
            ...
            // render as a bare "LeoPhoneAgent" header bubble with the typing
