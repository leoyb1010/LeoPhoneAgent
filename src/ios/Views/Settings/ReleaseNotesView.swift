...
enum LeoReleaseNotes {
    static let all: [LeoReleaseNote] = [
        LeoReleaseNote(
            ...
            date: "2026-08-11",
            items: [
                "🧭 独立 iPhone Agent 体验升级正式入册:首页任务卡直达 Mac 执行、聊天页 @ 引用文件、网关设置翻新(此前随 88 版装上但未在更新说明里交代,本条补上)",
                "🔐 凭证审批补上第二条通道:此前 shell 里的 minis-browser-use 命令读写 Cookie 不经审批弹窗,审批闸已下沉到所有调用路径的总收口,任何通道一视同仁",
                "🌐 审批弹窗显示真实站点:「本会话允许」现在按真实域名记账,给 A 站的授权不再顺带放行 B 站",
                "🛡 审批队列不再互相打架:并发多条审批时,点掉第一条不会误裁决还没展示的第二条;人在会话列表时触发的审批也能弹出来,不再无声挂起",
                "🕸 链接预览防内网探测加固:识别十进制/八进制 IP、IPv6 映射地址、CGNAT 段,并对域名做解析预检,恶意链接更难借预览摸内网",
                "📝 笔记保存基线修正:自动保存后又改回原文再退出,不会再把中间态永久留盘;删除笔记与自动保存的竞态不再产生孤儿文件",
                "📚 藏宝阁与分享扩展并发写入加了跨进程协调,分享进来的条目不会被主 app 的后台流水线覆盖丢失",
                "⚡ 藏宝阁列表滚动更顺:文件缩略图改为后台生成,不再在主线程解码原图",
                "🖥 Mac 端(LeoAgent 服务)同步加固:grok 登录并发死锁、慢连接静默断流、APNs 推送重试、审批横幅互顶等 8 项修复",
            ]),
        LeoReleaseNote(
            version: "1.21.0",
            date: "2026-08-11",
        ...
        LeoReleaseNote(
            ...
            date: "2026-08-11",
            items: [
                "🚨 修复锁屏审批按钮不生效:推送通知上的「批准一次/拒绝」此前点了没反应(通知里缺主机标识,匹配永远落空)——app 被杀时这是唯一的审批通道,现已按机器名逐级回退匹配",
                "🚨 修复写批注会抹掉该条的全文检索数据(不可逆):批注保存时的索引重建会把已抓取的文章正文替换成两行字。现在批注只走内存搜索,不碰全文索引",
                "🛟 后台补抓封面不再覆盖你同时做的置顶/批注/归档(改为只写抓到的字段);补抓改为全部完成后刷新一次列表,不再一分钟内十次全表刷新",
                "🎨 空状态撤掉两个大按钮,页面留给内容;入口收敛为右上角 + 与剪贴板条",
                "🐛 一批小修:新建笔记时抢先打的字不再被磁盘加载覆盖 · 「正在导入」提示不再中途消失 · 归档/批注现在会触发与 Mac 的同步 · 抓预览改流式读取到 512KB 即停 · 同一会话的连续推送折叠成一条横幅 · 负载均衡分组下胶囊如实显示分组名(不再谎报具体模型)· /model 在排队时也生效 · 同名模型挂两个供应商时会提示选哪个",
            ]),
        LeoReleaseNote(
            version: "1.20.4",
            date: "2026-08-10",
            items: [
                "🎨 藏宝阁界面翻新:卡片重排(标题更清楚、来源徽标按来源配色、时间改「3 分钟前」式相对时间)、笔记条目带角标、批注以引用样式显示在卡片里、筛选胶囊选中实底、剪贴板条重画、空状态图标带动效",
                "🫧 动效补齐:置顶跳到最前、归档滑走、删除收起都走弹簧过渡,不再瞬间跳变;提示气泡改材质胶囊从底部弹入",
                "🐛 修复新对话的模型胶囊显示错误:之前显示的是一个遗留字段里的模型(如 Haiku),而实际发送会用默认模型分组解析出的模型——显示的和用的不是同一个。现在胶囊显示真正会用的那个",
                "🖥 Mac 端 1.66.1 同步:镜像页认识笔记类型(带标识)、显示批注、手机上归档的条目同步收起",
            ]),
        LeoReleaseNote(
            version: "1.20.3",
            date: "2026-08-10",
            items: [
                "🚨 修复会清空笔记的严重问题:点开一条笔记、在正文还没读出来时就关掉,会把整篇写成空白。这是上一版「修复」自己带进来的(守卫条件在重构时漏掉了),现已修正,并且只读打开不再改动笔记的排序位置",
                "🚨 修复摘要与标签必被抹掉:生成摘要后再敲一个字,摘要就没了——保存时用的是打开那一刻的旧快照。现在一律以库里最新版为基底,编辑期间在别处改的置顶/归档/批注也不会被回滚",
                "🚨 修复后台补抓覆盖前台编辑:补抓元数据能跑一分多钟,期间你的编辑、导入、删除都可能被它写回的旧数据整份覆盖。收藏库的读-改-写现在是不可分割的",
                "🧹 删除条目时一并清掉笔记正文与历史版本(以前只删索引,文件永远留在容器里,容量只涨不降)",
                "🔗 分享纯链接不再收藏成两条;错误页/风控页的标题不再覆盖已有好标题;抓预览不再把几十 MB 的直链整个下下来",
                "⏱ Siri 记笔记的超时现在真的封顶(原来的写法会隐式等慢任务跑完)",
            ]),
        LeoReleaseNote(
            version: "1.20.2",
            date: "2026-08-10",
