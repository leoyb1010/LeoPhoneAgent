...
struct CloudSyncSettingsView: View {
    ...
    var body: some View {
        ...
        .alert("Are you absolutely sure?", isPresented: $showDeleteCloudStep2) {
            ...
            Text("This action cannot be undone. All LeoPhoneAgent data stored in iCloud will be erased immediately. Other devices signed into the same iCloud account will lose their synced copies on the next sync.\n\nLocal data on this device is safe.")
        }
        .alert("iCloud Data Deleted", isPresented: $showDeleteCloudDone) {
            Button("OK", role: .cancel) {}
        } message: {
            Text("All LeoPhoneAgent data has been removed from iCloud. This device is now re-uploading its local content to a fresh iCloud zone.")
        }
    ...
    private var dangerZoneSection: some View {
        Section {
            ...
            Text("Permanently erase all LeoPhoneAgent data from iCloud. Local data on this device is not affected.")
        }
    }
