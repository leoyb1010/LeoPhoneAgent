...
enum LeoReleaseCatalog {
    static let releases: [LeoRelease] = [
        ...
        LeoRelease(
            ...
            highlights: [
                "审批更省心:输入框上方的开关变成三档——逐项确认、智能批准、全自动。智能批准直接放行只读、不碰个人数据的操作(查看文件、git status、天气、地图这类),会改动的先问你,同一个会话里允许过就不再问;删除、强推、sudo 这类命令会在审批框上标成高风险,只提示、不加步骤。锁屏和手表上的审批通知直接给「允许一次 / 本次会话允许 / 拒绝 / 拒绝并停止任务」,不用先解锁;手机能力的审批框多了「始终允许」。",
                "iPad:有了菜单栏(按住 ⌘ 看快捷键:⌘N 新任务、⌘F 搜索、⌘K 命令面板、⌘1–9 切会话、⌘. 停止、⌥⌘I 检查器);聊天右边的检查器列出每一步工具调用、会话信息、产物和文件,竖屏时不再盖住输入框;会话右键「在新窗口打开」,或把会话拖到屏幕边缘开新窗口,关掉还有任务在跑的窗口前会先问你(iPadOS 27);设置在宽屏上变成左右两栏;链接和选中的文字可以直接拖进输入框,回复里的图片可以拖到文件、备忘录。",
                "聊天更顺:做完的一轮把中间步骤收成一行「已工作 N 步 · 用时」,点一下展开;思考块写明「思考了 N 秒」;只有正在输出的文字逐词淡入,代码、图片、公式直接出现,打开「减弱动态效果」或系统要求省电时不做动画;工具条出现时不再把正在往上看的你拽回底部;流式输出时少了几百次主线程切换,正在输出的表格不再每来一个字就整张重排;发送、排队、停止合成一个会变形的按钮,⌘↩ 只发送、不会误停任务;iOS 27 上往下读回答时导航栏自动收起。",
                "灵动岛和通知:任务等你批准时灵动岛变橙并提醒一下(戴着手表会震);App 超过 5 分钟没更新时显示「可能已中断」,不再永远「运行中」;手表 Smart Stack 有专门的小尺寸样式;iOS 27 上灵动岛变窄时只显示数字;隐私模式开关立即生效;任务完成的通知里可以直接回复接着说;正在看的会话不再弹横幅。",
                "Apple Watch:朗读更有分寸——放下手腕暂停、抬起继续,只念最多 4 句,念完把音乐音量还回去,「记录」页可关自动朗读;直连模型时中途放下手腕,回答转到后台继续,答完用通知送来;审批卡按风险着色,双指互点就是「允许一次」;从 Siri 或操作按钮「问 Leo」在 App 已经打开时也会开始听;iPhone 上有任务在跑时表上能看到。",
                "连 Mac 和 Siri:Mac 暂时不在线时发给它的消息先在中继排队,上线后自动送达(中继升级后生效);快捷指令「发送提示并等待结果」在 iOS 27 上不再 30 秒左右被掐断;没有屏幕的 Siri(AirPods、CarPlay)会得到适合朗读的短回答;接管浏览器后直接关掉面板,Agent 会接着做,不再一直干等。",
                "发版前自查修掉的问题(用起来):锁屏上的「本次会话允许」对 Mac 上的 Claude Code、Codex、Grok 任务只管这一个会话,不会变成永久放行;智能批准不再放过多行脚本和 $( ) 这类夹带别的命令的写法;审批弹框盖在最上层,不会把开着的设置、控制台关掉;回答中途断流或到输出上限时,红框里给的是「继续」,不会在半截回答下面再写一份;iPad 横屏弹出键盘时两栏不再塌成一栏,⌘K 按 Esc 能关、回车打开第一项;点 Mac 的通知直接打开那个会话;界面里残留的英文和内部代号换成了中文。",
                "发版前自查修掉的问题(不丢不乱):任务进行中排队的追问,回复不再写进上一条回答里,也不再丢;长任务中途自动压缩上下文时,停止按钮和灵动岛不再以为任务已经结束;App 在工具跑到一半时被系统关掉,已经说过的话和跑完的结果都还在,点「继续」不会把执行过的命令再跑一遍;锁屏后后台时间用完,回到 App 任务接着跑,不再算成你手动停止;删掉的对话不会在 iPad、Mac 上复活;iCloud 强制拉取会读完整个对话,运行中的对话不让拉;在 Mac 任务里点停止之后再发消息,手机会接着跟;Mac 上线后拒收排队消息时会告诉你并把原文放回输入框;远程主机网络一断,不会再删掉你存的 SSH 密码;Siri「停止 Mac 任务」停的是正在跑的那个。"
            ]
        ),
        LeoRelease(
            version: "1.43.0",
            date: "2026-09-25",
            title: "灵动岛不再误报失败;iPad 打开就能交代任务;手表离开 iPhone 也能答",
            highlights: [
                "灵动岛和后台说真话:任务还在跑时不再显示「失败」;长任务不会因为进度停住被系统当成卡死而报失败;暂停的任务显示「已暂停，回到 App 继续」,被系统打断的显示「已中断，回到 App 继续」,不再打一个完成的勾;失败或在等你处理的显示橙色感叹号和「需要注意」,你自己停掉的显示灰色,只有真正做完才是绿色对勾。工具名和状态也换成中文(读取文件、正在回复、思考中…)。",
                "收藏模型:删掉服务商、模型或分组时,它的收藏和最近使用一起清掉,不再看不见却占着 6 个名额;以前删服务商留下的这种「隐形收藏」不再算名额,下次点星时顺手清掉。默认分组、Agent 可调用的模型分组、压缩和起标题用的模型也跟着回落或清空;聊天里的模型胶囊显示发送时真正会用的模型,没配置模型时如实写「未选择模型」。",
                "iPad:打开不再是一片空白。没选对话时右边就是首页工作台,和 iPhone 首页同一条输入栏(执行位置 · 模型、/、麦克风),竖屏侧栏收起时也能直接交代任务;「/」面板在 iPad 上能打开了(以前点了没反应);设置、藏宝阁、Mac 控制台改成整页面板,不再挤在屏幕中间的小卡片或窄侧栏里;开着对话时,侧栏右上角的「新任务」回到工作台。",
                "Apple Watch 只管说话:点一下说,答案显示在表上,可以朗读、可以追问,问过的留在「记录」页。iPhone 在身边时交给 iPhone 上的 Leo 完整处理;蜂窝版手表离开 iPhone 时,直接用默认分组里的模型回答(设置 → Siri 指挥中心 → Apple Watch 可关,钥匙只存在手表的钥匙串里)。放下手腕后动画自动停下,更省电。",
                "更稳:启动时不再在主线程同步等定位服务回话(没有地点自动化时不再查询已登记的地点),定位服务慢的时候不会卡在启动页;新对话的起手式不再被输入框压住。另外清掉了 1500 多行没人调用的旧代码。"
            ]
        ),
        LeoRelease(
            version: "1.42.4",
            ...
            title: "设置逐项说到做到",
            highlights: [
                "说到做到:应用锁显示的时长和实际一致(默认离开 1 小时内回来不用再刷脸,以前页面上写的是「退出时锁定」);关掉「位置追踪」后台就真的不再用定位(以前开着「增强后台运行」照样用;没动过这个开关的,按以前的实际情况保持开着);Soul 里选的回复语言会真的告诉模型;推理规则里从没生效过的「默认档」去掉了。",
                "权限:权限页、能力中心、详情页显示同一个值,改一处其他地方跟着变;统一叫「自动允许 / 先问我 / 不允许」(「先问我」=同一会话里批过一次就不再问);「全部自动允许」要先确认。",
                "少走弯路:设置首页新增「后台与通知」,搜「后台」「通知」「灵动岛」「定位」「语言」「字体」都找得到;删掉的 Mac 下拉刷新不会再冒出来;文件夹的「允许写入」「在文件 App 中显示」开关立即生效,不用再点保存;自动化规则被 3 个 👎 暂停后可以右滑恢复,新建地点规则时会说明为什么还不能保存,授权后自动重新定位;AI 服务商登录失败的原因显示在登录按钮下面,自己点取消不再报错。",
                "说中文:聊天输入框上方的任务状态卡(正在用工具、需要你批准、可以继续……)、活动记录和 Agent 时间线、Apple 系统能力列表、隐私与数据页、添加服务商前的数据说明,原来是英文或直接显示内部代号,现在都是中文;首页「N 个会话等你处理」也改成中文;「主动卡」开关改名「首页显示 Mac 进行中」,说明和实际一致。",
                "安全:能力自检免审批只认自检自己的几条只读探测,同一会话里的其他命令照常审批。"
            ]
        ),
        LeoRelease(
            version: "1.42.3",
            date: "2026-09-25",
            title: "修好悬浮工具条;自检免审批收紧",
            highlights: [
                "修好对话底部的悬浮工具条跟不上正在跑的工具(1.42.1 引入):新对话第一次用工具时它不出现,之后一直停在上一轮的最后一个工具。",
                "能力自检的免审批收紧:只在自检进行的那一会儿、只对自检自己的探测命令生效;这个保留的会话不能再被命令行或链接拿来发消息、打开。",
                "供应商页:改完名字、接口地址、User-Agent 直接返回,几项不再互相覆盖。",
                "打开新对话时照旧清掉 pip 的 EXTERNALLY-MANAGED 标记,apk upgrade 之后 pip install 不会失败;快捷指令或定时任务正在跑时,被拒绝的发送不会把它在活动记录里的来源改成「App」。"
            ]
        ),
        LeoRelease(
            version: "1.42.2",
            date: "2026-09-25",
            title: "自检不再干等;首页输入栏不再透字",
            highlights: [
                "能力自检:开着全自动时,日历、提醒事项、相册、定位、健康、正在播放、播放器、快捷指令这几项不再每项干等 25 秒、报「在等系统授权」。自检跑的是 App 内置的只读探测,按下「开始自检」就算同意;你设成「不允许」的能力照样不跑。",
                "首页输入栏后面垫了一层渐变到页面底色:列表滚到底下时在输入栏上方淡出,会话标题不再糊在占位字下面,输入栏下面也不再露出半截会话。",
                "启动后的第一个新对话也不再闪一下「Booting Kernel」:只有第一次安装、要解包 Linux 环境时才显示。",
                "用量页的「缓存命中率」、存储页的「Shell 容器 / 对话数据库 / 会话文件」显示成中文。"
            ]
        ),
        LeoRelease(
            version: "1.42.1",
            date: "2026-09-25",
            title: "更顺:新对话秒开,打字和语音不卡,键盘不再挡住最后一行",
            highlights: [
                "新对话秒开:除了启动后的第一个,新对话不再先闪一下「Booting Kernel」;从首页输入框发出的任务会滑进对话页,不再「啪」地切过去;已经在首页时点通知、Siri、深链打开会话不再白等 0.3 秒,设置等面板开着会先收起再打开。",
                "打字和语音更顺:首页输入框和搜索框打字不再每个字都把整个会话列表重算一遍;搜索第一个字不再把全部历史摊开又收回;语音输入时只刷新波形(最多每秒 30 次),聊天页不再跟着每个音频片段重画;点聊天输入框时最后几行跟着键盘一起上移,不再先被挡住、再滚一次。",
                "后台更省:回到 App 的第一帧不再同步查钥匙串;挂载目录的可写检查挪到后台;技能 30 秒内不重复扫描;Agent 跑工具时首页和会话行不再每一步都重画;活动记录每 200 条才整理一次;藏宝阁每次只筛一遍。",
                "供应商页:钥匙在你停手 0.8 秒或离开页面时才保存,只是打开页面不再重写钥匙串(以前会刷新保存时间,本机的旧钥匙可能在 iCloud 同步里盖掉别的设备上新换的);名字在回车或离开时保存;供应商列表不再每次重画都查钥匙串。",
                "修复:首页列表文字不再透过输入栏和占位字叠在一起,iPad 侧栏也换成同一条输入栏;能力自检真发一句话只挑能聊天的模型;ChatGPT 模型目录暂时拉不到时保留现有模型并说明原因;手机关着全自动时,发给 Mac 任务的消息会让它切回先问你,接上 Mac 上空闲的任务直接显示空闲,建任务失败时第一句话放回输入框;快捷指令发起的任务在活动记录里标对来源;Android 机器的「指挥」菜单只列它自己的 Agent。"
            ]
        ),
        LeoRelease(
            version: "1.42.0",
            date: "2026-09-25",
            title: "全自动一点即开;ChatGPT 登录能用上 GPT-6",
            highlights: [
                "全自动不用再去设置里开:聊天和首页的输入框上方多了一个开关,显示「逐项确认」时点一下就切到「全自动」,再点一下关掉;第一次打开会先说明一次后果,之后一点即开。也可以在输入框里发 /auto 切换。",
                "修好 ChatGPT(OpenAI 登录)读不到新模型:以前用的是内置的固定清单,新出的 GPT-6-Astra / Sol / Luna 一直出不来;现在直接读 OpenAI 的实时模型目录,和 Codex 看到的一致,思考强度的上限也跟着目录走。装好后会自动刷新一次;离线时退回内置清单(已补上 GPT-6)。",
                "能力自检可以顺便真发一句话测模型(页面上的开关,默认关):每个供应商花几十个 token,用和聊天完全相同的请求路径,模型坏了能直接看到服务端原话。",
                "诊断更准:被系统预先拉起的启动不再算成几小时的冷启动;发送到首字的耗时会记下用的是哪个模型、模型先想了多久;在多任务里划掉 App 不再记成崩溃。"
            ]
        ),
        LeoRelease(
            version: "1.41.2",
