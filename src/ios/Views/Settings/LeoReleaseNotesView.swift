...
enum LeoReleaseCatalog {
    static let releases: [LeoRelease] = [
        LeoRelease(
            ...
            highlights: [
                "审批更省心:输入框上方的开关变成三档——逐项确认、智能批准、全自动。智能批准直接放行只读、不碰个人数据的操作(查看文件、git status、天气、地图这类),会改动的先问你,同一个会话里允许过就不再问;删除、强推、sudo 这类命令会在审批框上标成高风险,只提示、不加步骤。锁屏和手表上的审批通知直接给「批准一次 / 本次会话允许 / 拒绝 / 拒绝并停止任务」,不用先解锁;手机能力的审批框多了「始终允许」。",
