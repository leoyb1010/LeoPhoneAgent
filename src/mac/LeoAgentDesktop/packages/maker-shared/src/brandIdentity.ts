 ...
 * 已分目录无需再区分)。
 */
// LeoAgent 身份(2026-08-06 分叉自上游 Cindy)。这是一个独立产品的独立身份:
// bundle id、scheme、userData 目录、DB 前缀全部换成我们自己的,与上游不共享
// 任何系统注册位,两者可同机并存互不干扰。
//
// legacy 数组按上游语义是「只增不减的历史兼容锚点」,但那些锚点属于上游产品的
// 存量用户,不属于本产品——保留它们会让本 app 去认领 Cindy 的深链与数据目录。
// 因此这里清空:本产品没有存量,第一版即是起点。
export const BRAND_IDENTITY: BrandIdentity = Object.freeze({
  displayName: BRAND_NAME,
  executableName: 'LeoAgent',
  ...
  appIdByRegion: Object.freeze({
    cn: 'com.leoyuan.leoagentcn',
    global: 'com.leoyuan.leoagent',
    dev: 'com.leoyuan.leoagentdev',
  }),
  primaryScheme: 'leoagent',
  legacySchemes: Object.freeze([]),
  userDataDirName: 'LeoAgent',
  userDataDirNameByRegion: Object.freeze({
    cn: 'LeoAgent',
    global: 'LeoAgentGlobal',
    dev: 'LeoAgentDev',
  }),
  legacyUserDataDirNames: Object.freeze([]),
  cdnPrefix: 'leoagent',
  updaterName: 'leoagent-updater',
  dbFilePrefix: 'leoagent',
