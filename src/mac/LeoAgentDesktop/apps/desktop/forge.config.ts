...
function applyMacPackagedDisplayName(buildPath: string, platform: string): void {
  ...
  for (const appDir of apps) {
    ...
    // 展示名走单一事实源,不再写死字面量:写死会让 branding.ts 改名后 Dock /
    // Cmd+Tab / 通知仍显示旧品牌(本次分叉就踩到了这一条)。
...
function extraResourcesForTarget(targetPlatform: string): string[] {
  const base = [
    'resources/icon.png',
    'resources/tools',
    // 端点清单随包内置(LeoAgent 分叉:不依赖远端配置服务,见
    // clientEndpointsService.resolveEndpointSource)。
    'resources/endpoints',
    // 本地更新记录(个人版没有公告 CDN,changelog 随包)。
...
const makers: ForgeConfig['makers'] = [
  ...
  new MakerDeb({
    options: {
      ...
      maintainer: 'Leo Yuan <leoyb1010@gmail.com>',
  ...
  packagerConfig: {
    ...
    extendInfo: {
      ...
      CFBundleDocumentTypes: [
        ...
        {
          CFBundleTypeName: 'LeoAgent Cartridge',
