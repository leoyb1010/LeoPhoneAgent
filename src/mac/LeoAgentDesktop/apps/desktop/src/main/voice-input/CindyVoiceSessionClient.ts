...
export class CindyVoiceRunContext {
  ...
  async createRefinerTarget(
    ...
    if (this.refinerUnavailableOnServer) {
      throw new Error('LeoAgent voice service on this server does not support managed refinement yet.');
    }
    ...
    if (!token) throw new Error('LeoAgent login is required for voice refinement.');
    const baseUrl = getClientEndpoint('voiceApiBaseUrl');
    if (!baseUrl) throw new Error('LeoAgent voice service is unavailable in this region.');
    return {
...
async function createCindyVoiceSession(input: {
  ...
  requireAppCapability('canUseCindyAccountServices', 'LeoAgent voice requires a Cindy account.');
  if (!getClientEndpoint('voiceApiBaseUrl')) {
    throw new Error('LeoAgent voice service is unavailable in this region.');
  }
  const request = {
  ...
  if (
    ...
    throw new Error('LeoAgent voice service returned an invalid session.');
  }
  return session;
}
