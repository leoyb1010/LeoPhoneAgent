...
export interface ResolveEndpointSourceInput {
  isPackaged: boolean;
  env: {
    /** '1' = 也走完整 CDN 拉取(index.ts 已把 --endpoints-cdn 收敛到该 env)。 */
  ...
  repoRoot: string;
  /** packaged 下的 resources 目录(process.resourcesPath)。 */
...
export function resolveEndpointSource(input: ResolveEndpointSourceInput): EndpointSource {
  // LeoAgent 分叉:packaged 也读随包内置的清单,不依赖远端配置服务。
  // 严格解析语义不变,只是事实源从 CDN 换成 app 包内。
  ...
  const base = input.isPackaged
    ? path.join(input.resourcesPath ?? '', 'endpoints')
    : path.join(input.repoRoot, 'config');
  const filePath = override
    ? path.resolve(input.isPackaged ? base : input.repoRoot, override)
    : path.join(base, MANIFEST_FILE_NAME);
