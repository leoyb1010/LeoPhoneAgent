 ...
 */
// ── LeoAgent:本地更新记录(个人版没有公告 CDN)────────────────────────────
// 发版时把 changelog 随包放进 resources/release-notes/{platform}/,启动后的
// 版本比对与弹窗链路完全复用上游——只是"事实源"从远端换成了包内文件。

function localNoticeDir(): string {
  const base = app.isPackaged
    ? process.resourcesPath
    : path.resolve(app.getAppPath(), 'resources');
  return path.join(base, 'release-notes', getPlatformKey());
}

function readLocalNotice(version: string): RawReleaseNotes | null {
  try {
    const raw = fs.readFileSync(path.join(localNoticeDir(), `${version}.json`), 'utf8');
    return JSON.parse(raw) as RawReleaseNotes;
  } catch {
    return null;
  }
}

function readLocalIndex(): string[] | null {
  try {
    const raw = fs.readFileSync(path.join(localNoticeDir(), 'index.json'), 'utf8');
...
export async function fetchReleaseNotes(version: string): Promise<RawReleaseNotes | null> {
  const local = readLocalNotice(version);
  if (local) {
    cache.set(version, local);
    return local;
  }
  return fetchReleaseNotesRemote(version);
}

async function fetchReleaseNotesRemote(version: string): Promise<RawReleaseNotes | null> {
