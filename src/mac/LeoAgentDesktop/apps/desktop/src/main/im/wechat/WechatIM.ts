  ...
  async #processCommand(task: WechatTask, command: string): Promise<void> {
    ...
    switch (command) {
      ...
      case '/status':
        await this.#commitSimpleReply(
          task,
          `LeoAgent 微信连接正常，当前队列 ${this.#state.queuedTasks} 条。`,
        );
        return;
      case '/stop':
      case '/stop all':
  ...
  async #commitPreDispatchFailure(task: WechatTask, reason: string): Promise<void> {
    const text =
      ...
      reason.includes('unsupported_turn_permission')
        ? '当前 Agent 无法在个人微信中安全使用此权限模式。请在 LeoAgent 中切换权限模式；若仍失败，请改用支持个人微信权限确认的 Agent。'
        : reason === 'missing_auth'
          ? '当前 Agent 尚未完成授权，请先在 LeoAgent 中连接模型服务。'
          : '这条消息暂时无法启动，请稍后重试。';
