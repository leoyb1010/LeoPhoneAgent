...
const CODEX_COMPUTER_USE_REPLACEMENT_ROUTE = {
  ...
  reason: 'LeoAgent owns desktop-control enablement, permissions, and execution.',
  ...
  overrides: [
    {
      ...
      reason: 'Natural-language Feishu requests should use the LeoAgent-connected account.',
    },
    {
      capabilityId: 'feishu',
      source: {
    ...
    // user enabled Cindy's own Computer Use replacement.
    {
      ...
      invocation: 'disabled',
      reason: 'The bundled Skill requires node_repl, which is unavailable in LeoAgent.',
    },
  ],
...
const CODEX_IN_APP_BROWSER_UNAVAILABLE_OVERRIDE = {
  ...
  invocation: 'disabled',
  reason: 'LeoAgent does not host the ChatGPT in-app browser runtime.',
...
const CODEX_CHROME_USE_OVERRIDES = [
  {
    ...
    reason: 'LeoAgent Browser owns browser automation while it is enabled for this workspace.',
  },
  {
    capabilityId: 'browser-use',
    source: {
