...
export function loadSqliteVec(db: Database.Database): SqliteVecLoadResult {
  ...
  try {
    try {
      ...
      // SQLite 的 load_extension 在拿不到入口点时会自己补平台后缀,于是带后缀的
      // 路径被拼成 `vec0.dylib.dylib` 而 dlopen 失败(报错原文里看得见双后缀)。
      // 去掉后缀再试一次,让 SQLite 自己补。两次都失败才算真失败。
      const stripped = extPath.replace(/\.(dylib|so|dll)$/i, '');
      if (stripped === extPath) throw first;
