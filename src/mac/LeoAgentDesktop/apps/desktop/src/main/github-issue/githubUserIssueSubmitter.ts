  ...
  if (!operation.ok) {
    if (isGithubAuthFailure(operation.message)) {
      throw submissionError(
        'AUTH_NOT_READY',
        `已绑定的 GitHub 身份不可用，issue 未提交。请到「插件」→「LeoAgent GitHub」重新配置并测试连接：${operation.message}`,
      );
    }
...
export async function postGithubIssueAsUser(
  ...
  if (
    ...
    throw submissionError(
      'SERVER_ERROR',
      'LeoAgent GitHub 返回的 issue 创建结果不完整，issue 状态无法确认，请勿自动重试。',
    );
  }
...
async function requireGithubOperation(
  ...
  try {
    ...
    throw malformedResponseError(`LeoAgent GitHub 的 ${name} 调用失败`, err);
  }
  ...
  throw submissionError(
    code,
    code === 'AUTH_NOT_READY'
      ? `GitHub 用户身份或仓库权限不可用，issue 未提交。请到「插件」→「LeoAgent GitHub」检查 token 权限：${operation.message}`
...
async function callGithubOperation(
  ...
  const result = await withOptionalTimeout(
    ...
    () => ({
      ...
      message: `LeoAgent GitHub ${name} 超时`,
    }),
  );
...
function parseGithubLogin(value: unknown): string {
  if (!isRecord(value) || typeof value.login !== 'string' || !value.login.trim()) {
    throw submissionError(
      'SERVER_ERROR',
      'LeoAgent GitHub 未返回有效的 GitHub 用户名，issue 未提交。',
    );
  }
