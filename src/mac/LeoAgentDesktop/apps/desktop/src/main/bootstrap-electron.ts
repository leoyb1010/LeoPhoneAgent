...
import { registerLeoFleetIpc } from './leoFleet.js';
import {
  getGhostCindySlot,
  getGhostManager,
...
// macOS Finder "打开方式 → LeoAgent" 入口:声明 CFBundleDocumentTypes 接受
...
// cindy://session 等)/ 右键 "通过 LeoAgent 打开" 而拉起的第二个进程 redirect 成
...
if (
  ...
  if (!gotTheLock) {
    markDesktopDevStartupFailed(
      ...
      'Another LeoAgent instance already owns this single-instance lock scope.',
      { userDataDir: realUserDataDir, lockScopeDir },
    );
    app.quit();
  } else {
    app.on('second-instance', (_event, argv) => {
      // Windows: 用户点 cindy://(或历史 xdt-maker://)链接 / 右键 "通过 LeoAgent 打开" 时,
...
// 冷启动 argv 扫描 — Windows 上首次点链接 / 右键 "通过 LeoAgent 打开" 启动 app
...
const registerIpcHandlers = () => {
  ...
  registerBillingIpc({
    ...
    requirePersonalAccount: () => {
      requireAppCapability(
        ...
        'Billing requires a personal LeoAgent account.',
      );
  ...
  ipcMain.handle(MAKER_IPC_INVOKE.CHAT_EMBEDDING_GET, async () => {
    requireAppCapability('canUseCindyGateway', 'Chat embedding requires a LeoAgent account.');
  ...
  const requireCloudProfile = (): void => {
    if (!getAppCapabilities().canUseCindyAccountServices) {
      throwIpcError('PERMISSION_DENIED', 'Profile editing requires a LeoAgent account.');
    }
  };
  ...
  // 否则不带标签（splash 显示单一 "唤醒 LeoAgent 中..." 文案）。
