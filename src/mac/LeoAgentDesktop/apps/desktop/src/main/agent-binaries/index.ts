...
import { findSystemCliPath, resolveSystemCli } from './system-cli-fallback.js';
import {
...
export async function prepare(
  ...
  if (!app.isPackaged) {
    ...
    // dev 下同样接受系统 CLI:开发机上装了 claude/codex 就该直接能跑。
    const devSystem = await resolveSystemCli(kind);
    if (devSystem) {
      lastReadyPath.set(kind, devSystem.path);
      return { ready: true, path: devSystem.path, downloaded: false };
    }
    return { ready: false, error: `${kind} dev binary not found for ${getPlatformKey()}`, downloaded: false };
  }

  // ── LeoAgent:系统已装 CLI 优先(所有平台,含 packaged)────────────────────
  // 本产品驱动的是用户自己装好、自己登录好的 CLI。排在任何下载链之前:上游那条
  // CDN 我们不运营,拉不到就等于「环境初始化失败」,而机器上其实早就装好了。
  // 只在 async 路径调用——同步的 getCachedBinaryStatus 里做 PATH 扫描会把主进程
  // 卡在建窗之前(上游注释明确警告过)。
  {
    const systemCli = await resolveSystemCli(kind);
    if (systemCli) {
      console.log(`[agent-binaries/${kind}] system CLI: ${systemCli.path} (${systemCli.version})`);
      lastReadyPath.set(kind, systemCli.path);
      ...
      return { ready: true, path: systemCli.path, downloaded: false };
    }
  }
...
export async function peekNeedsDownload(kind: AgentBinaryKind): Promise<boolean> {
  // 系统已装的 CLI 会被 prepare 直接采用,不需要下载——peek 必须同口径,
  // 否则 splash 会为一个根本不会发生的下载显示进度分段。
  if (findSystemCliPath(kind)) return false;
