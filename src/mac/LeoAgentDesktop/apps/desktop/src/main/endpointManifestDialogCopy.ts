...
> = {
  'zh-CN': {
    networkTitle: 'LeoAgent 暂时无法连接',
    configTitle: 'LeoAgent 暂时无法启动',
    networkBody:
      '启动时未能连接到 LeoAgent 服务。请确认设备已联网，然后重新尝试启动。如果正在使用 Proxy 或公司网络，可以切换网络后再试。',
    configBody: 'LeoAgent 暂时无法完成启动。请稍后重新尝试；如果问题持续出现，请联系技术支持。',
    ...
    retryButton: '重试启动 LeoAgent',
    offlineButton: '用上次配置启动',
    quitButton: '退出 LeoAgent',
  },
  en: {
    ...
    noSavedConfigurationHint:
      'This device has no saved configuration yet, so LeoAgent needs to reconnect before it can start.',
    offlineHint:
      'You can also start offline with the configuration LeoAgent last fetched ({{savedAt}}). Features that need a network connection will be unavailable.',
    retryButton: 'Try Starting LeoAgent Again',
    offlineButton: 'Use Last Configuration',
    quitButton: 'Quit LeoAgent',
  },
  ja: {
    networkTitle: 'LeoAgent に一時的に接続できません',
    configTitle: 'LeoAgent を一時的に起動できません',
    networkBody:
      '起動時に LeoAgent のサービスへ接続できませんでした。デバイスがネットワークに接続されていることを確認して、もう一度起動してください。プロキシや社内ネットワークを使用している場合は、ネットワークを切り替えてお試しください。',
    configBody:
      'LeoAgent の起動を完了できませんでした。時間をおいてもう一度お試しください。問題が続く場合は、サポートにお問い合わせください。',
    ...
    retryButton: 'LeoAgent の起動を再試行',
    offlineButton: '前回の設定で起動',
    quitButton: 'LeoAgent を終了',
  },
  ko: {
    networkTitle: 'LeoAgent에 일시적으로 연결할 수 없습니다',
    configTitle: 'LeoAgent를 일시적으로 시작할 수 없습니다',
    networkBody:
      '시작하는 동안 LeoAgent 서비스에 연결하지 못했습니다. 기기가 네트워크에 연결되어 있는지 확인한 후 다시 시작해 보세요. 프록시나 회사 네트워크를 사용 중이라면 다른 네트워크로 전환해 보세요.',
    configBody:
      '지금은 LeoAgent 시작을 완료할 수 없습니다. 잠시 후 다시 시도해 주세요. 문제가 계속되면 지원팀에 문의해 주세요.',
    ...
    retryButton: 'LeoAgent 시작 다시 시도',
    offlineButton: '지난 설정으로 시작',
    quitButton: 'LeoAgent 종료',
  },
};
