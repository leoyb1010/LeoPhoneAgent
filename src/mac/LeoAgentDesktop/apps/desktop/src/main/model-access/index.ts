...
export async function refreshXdGatewayModels(): Promise<void> {
  if (!getAppCapabilities().canUseCindyGateway) {
    throwIpcError('PERMISSION_DENIED', 'LeoAgent AI requires a Cindy account.');
  }
  const status = await ensureCredentialsReadyForModelsRefresh(getSync());
  if (status.state !== 'ok') {
    throwIpcError('MODEL_ACCESS_FAILED', 'LeoAgent AI credentials are not ready.');
  }
  const gen = authGeneration;
  ...
  switch (outcome) {
    case 'succeeded':
      return;
    case 'not-started':
      throwIpcError('MODEL_ACCESS_FAILED', 'LeoAgent AI model list refresh did not start.');
    case 'account-changed':
      throwIpcError('MODEL_ACCESS_FAILED', 'LeoAgent AI account changed during model list refresh.');
    case 'failed':
      throwIpcError('MODEL_ACCESS_FAILED', 'LeoAgent AI model list refresh failed.');
  }
}
