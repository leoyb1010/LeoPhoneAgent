  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: { code: string } | { error: string }) => {
      ...
      if (server.listening) {
        ...
        // rendering "return to LeoAgent". Closing belongs to cleanup; do not hold
...
export async function initialize(options: AuthInitializeOptions = {}): Promise<AuthState> {
  ...
  // LeoAgent:本产品**没有云端账号体系**,登录页在原理上永远不可能成功
  // (端点不指向任何真实 auth 服务器,只会显示"网络连接失败")。
  //
  // 因此启动**无条件**落成本地会话——不看持久化凭证:只在"零凭证"时才进
  // 本地的话,userData 里任何历史痕迹都会把用户扔回那个死登录页。
  // 写成编译期开关而非裸 return:裸 return 会让下方整段云登录逻辑变成
  // TS 眼里的死代码,原有的空值收窄全部失效,凭空冒出 11 个类型错误。
  // 将来自建 auth 服务时把开关翻回 true 即可回归完整登录链路。
  const CLOUD_AUTH_ENABLED = false as boolean;
