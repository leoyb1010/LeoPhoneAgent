...
export class GhostNetworkSlot {
        ...
        for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
          ...
          if (currentConnectionInjected.size > 0) {
            ...
            if (!stillCurrent) {
              return {
                ok: false,
                message: 'LeoAgent 企业账号已切换，本次请求已取消，请重试',
              };
            }
          }
  ...
  > {
    if (secret.source === 'oidc-token') {
      ...
      if (!manager) {
        return { error: 'LeoAgent 企业身份通道未就绪，请升级应用或反馈' };
      }
      let resolution: {
        membershipId: string;
        audience: string;
      ...
      try {
        ...
        return { error: 'LeoAgent 企业身份暂时不可用，请稍后重试或反馈' };
      }
      if (!resolution) {
        ...
        return { error: '当前 LeoAgent 企业身份不可用于此插件，请确认已登录正确的企业账号' };
      }
      if (!resolution.allowedHosts.includes(hostname)) {
        ...
        return { error: '当前 LeoAgent 企业身份不可用于此服务地址' };
      }
      const tokenInput = {
      ...
      try {
        ...
        return { error: '暂时无法获取 LeoAgent 企业身份，请检查网络后重试' };
      }
    }
