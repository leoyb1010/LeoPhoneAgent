...
import { TAB_GROUPS, TAB_IDS, TAB_KEYWORDS, TAB_LABEL_KEY, isSettingsTab } from '@/lib/tabLabels';
...
export function SettingsView() {
  ...
  // 设置搜索:标题 + 关键词表,大小写不敏感,空格分词全部命中才算匹配。
  const [settingsQuery, setSettingsQuery] = useState('');
  const filteredTabIds = useMemo(() => {
    const terms = settingsQuery.trim().toLowerCase().split(/\s+/).filter(Boolean);
    if (terms.length === 0) return visibleTabIds;
    return visibleTabIds.filter((tabId) => {
      const haystack = `${t(TAB_LABEL_KEY[tabId])} ${TAB_KEYWORDS[tabId] ?? ''}`.toLowerCase();
      return terms.every((term) => haystack.includes(term));
    });
  }, [settingsQuery, visibleTabIds, t]);
        ...
        >
          ...
          <div className="px-1 pb-2">
            <input
              ...
              placeholder="搜索设置…"
              aria-label="搜索设置"
              className="h-8 w-full rounded-full border border-[var(--sidebar-item-active-border)] bg-transparent px-3 text-sm text-[var(--settings-menu-text)] outline-none placeholder:text-[var(--settings-menu-text)]/50 focus:border-[var(--create-agent-focus-ring)]"
            />
          </div>

          {/* Tab list:搜索时扁平显示命中项;否则按分组渲染(个人版信息架构)。 */}
          <nav role="tablist" aria-label={t('settings.title')} className="flex flex-col gap-0.5">
            {(settingsQuery.trim()
              ? [{ label: '', tabs: filteredTabIds }]
              : TAB_GROUPS.map((g) => ({
                  label: g.label,
                  tabs: g.tabs.filter((id) => visibleTabIds.includes(id)),
                }))
            ).map((group, gi) => (
              <div key={group.label || `search-${gi}`} className="flex flex-col gap-0.5">
                {group.label && group.tabs.length > 0 && (
                  <div className="px-3 pb-1 pt-3 text-[11px] font-medium uppercase tracking-wide text-[var(--settings-menu-text)]/60">
                    {group.label}
                  </div>
                )}
                {group.tabs.length === 0 && settingsQuery.trim() && gi === 0 && (
                  <div className="px-3 py-2 text-sm text-[var(--settings-menu-text)]/60">
                    没有匹配的设置项
                  </div>
                )}
