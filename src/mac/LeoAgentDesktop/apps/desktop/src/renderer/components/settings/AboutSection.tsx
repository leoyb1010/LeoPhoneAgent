...
// 这是个人产品,没有社群渠道。上游那三条链接指向的是别人的社群,留着等于
// 把本产品的用户送到上游那里去——清空,关于页只展示版本与开源署名。
// 显式标注元素类型:空数组会被推成 never[],下面的 .map 里每个字段都报错。
interface DesktopSocialLink {
  id: string;
  labelKey: string;
  descriptionKey: string;
  url: string;
}
const DESKTOP_SOCIAL_LINKS: readonly DesktopSocialLink[] = [];
...
export function LegalLinksRows() {
  // LeoAgent 个人版:没有云端法务页,上游链接指向不存在的域名。开源许可与
  // 来源声明在仓库 NOTICE/LICENSE,这里不再渲染死链接。
  return null;
}
...
function SocialLinksPanel() {
  ...
  // 个人版没有社群渠道:无内容时整个板块(含标题)不渲染。
  if (DESKTOP_SOCIAL_LINKS.length === 0) return null;
