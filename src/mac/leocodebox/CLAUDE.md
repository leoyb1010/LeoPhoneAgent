...
Mac 这条链是三端里唯一从一开始就挂对位置的:闸门在 `desktop:dist:mac:signed`
链首,也就是发版真正会执行的那条命令上。iOS 那边的反面教材是闸门只挂在
GitHub Actions,而真实发版是本地 xcodebuild 直装真机,于是一次都没挡过 ——
定闸门先问"它在不在我真正会跑的那条路上"。详见仓库根 `CLAUDE.md`。
