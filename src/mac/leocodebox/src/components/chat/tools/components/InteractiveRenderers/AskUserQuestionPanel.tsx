...
export const AskUserQuestionPanel: React.FC<PermissionPanelProps> = ({
  ...
  return (
    <div
      ...
      className={`w-full outline-none transition-all duration-slow ease-out ${
        mounted ? 'translate-y-0 opacity-100' : 'translate-y-3 opacity-0'
      }`}
    >
      <div className="relative overflow-hidden rounded-xl border border-border bg-card shadow-elevation-2 dark:border-border dark:bg-muted/90 dark:shadow-elevation-3">
        {/* Accent line */}
        ...
        <div className="px-4 pb-2 pt-3.5">
          <div className="mb-1.5 flex items-center gap-2.5">
            ...
            <div className="relative flex-shrink-0">
              <div className="flex h-6 w-6 items-center justify-center rounded-lg bg-gradient-to-br from-blue-500/10 to-cyan-500/10 dark:from-blue-400/15 dark:to-cyan-400/15">
                <svg className="h-3.5 w-3.5 text-info dark:text-info" fill="none" viewBox="0 0 24 24" strokeWidth={1.75} stroke="currentColor">
            ...
            {/* Step counter */}
            {!isSingle && (
              <span className="flex-shrink-0 text-[10px] tabular-nums text-muted-foreground dark:text-muted-foreground">
                {currentStep + 1}/{total}
              </span>
            )}
          </div>
            ...
            <div className="mb-2 flex items-center gap-1">
              {questions.map((_, i) => (
                <button
                  ...
                  className={`h-[3px] rounded-full transition-all duration-slow ${
                    i === currentStep
                      ? 'w-5 bg-info dark:bg-info'
                      : i < currentStep
                        ? 'w-2.5 bg-info dark:bg-info'
          ...
          {/* Question text */}
          <p className="text-[14px] font-medium leading-snug text-muted-foreground dark:text-muted-foreground">
            {q.question}
          </p>
          {multi && (
            <span className="text-[10px] text-muted-foreground dark:text-muted-foreground">Select all that apply</span>
          )}
        </div>
          ...
          <div className="space-y-1">
            {options.map((opt, optIdx) => {
              ...
              return (
                <button
                  ...
                  className={`group flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-left transition-all duration-fast ${
                    isSelected
                      ? 'border-info bg-info/80 ring-1 ring-info/50 dark:border-info dark:bg-info/25 dark:ring-info/30'
                      : 'border-border hover:border-border hover:bg-muted/60 dark:border-border dark:hover:border-border dark:hover:bg-muted/40'
                  }`}
                >
                  {/* Keyboard hint */}
                  <kbd className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md font-mono text-[10px] transition-all duration-fast ${
                    isSelected
                      ? 'bg-info font-semibold text-primary-foreground dark:bg-info'
                      : 'border border-border bg-muted text-muted-foreground group-hover:border-border dark:border-border dark:bg-muted dark:text-muted-foreground dark:group-hover:border-border'
                  }`}>
                    {optIdx + 1}
                  </kbd>

                  <div className="min-w-0 flex-1">
                    <div className={`text-[13px] leading-tight transition-colors duration-fast ${
                      isSelected
                    ...
                    {opt.description && (
                      <div className={`text-[11px] leading-snug transition-colors duration-fast ${
                        isSelected
                          ? 'text-info/70 dark:text-info/70'
                          : 'text-muted-foreground dark:text-muted-foreground'
                      }`}>
                        {opt.description}
                      </div>
                  ...
                  {/* Selection check */}
                  {isSelected && (
                    <svg className="h-4 w-4 flex-shrink-0 text-info dark:text-info" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
            ...
            <button
              ...
              className={`group flex w-full items-center gap-2.5 rounded-lg border px-3 py-2 text-left transition-all duration-fast ${
                isOtherOn
                  ? 'border-info bg-info/80 ring-1 ring-info/50 dark:border-info dark:bg-info/25 dark:ring-info/30'
                  : 'border-dashed border-border hover:border-border hover:bg-muted/60 dark:border-border dark:hover:border-border dark:hover:bg-muted/40'
              }`}
            >
              <kbd className={`flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-md font-mono text-[10px] transition-all duration-fast ${
                isOtherOn
              ...
              {isOtherOn && (
                <svg className="ml-auto h-4 w-4 flex-shrink-0 text-info dark:text-info" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
                </svg>
              )}
            </button>
              ...
              <div className="pl-[30px] pr-0.5">
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-lg border-0 bg-muted px-3 py-1.5 text-[13px] text-muted-foreground outline-none ring-1 ring-border transition-shadow duration-base placeholder:text-muted-foreground focus:ring-2 focus:ring-info dark:bg-muted/60 dark:text-muted-foreground dark:ring-border dark:placeholder:text-muted-foreground dark:focus:ring-info"
                  />
                  <kbd className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md border border-border bg-muted px-1 py-0.5 font-mono text-[9px] text-muted-foreground dark:border-border dark:bg-muted dark:text-muted-foreground">
                    Enter
                  </kbd>
                </div>
              </div>
        ...
        {/* Footer — compact */}
        <div className="flex items-center justify-between gap-2 border-t border-border bg-muted/50 px-4 py-2 dark:border-border dark:bg-muted/50">
          <button
            type="button"
            onClick={handleSkip}
            className="text-[11px] text-muted-foreground transition-colors hover:text-muted-foreground dark:text-muted-foreground dark:hover:text-muted-foreground"
          >
            {isSingle ? 'Skip' : 'Skip all'}
            <span className="ml-1 text-[9px] text-muted-foreground dark:text-muted-foreground">Esc</span>
          </button>
          ...
          <div className="flex items-center gap-1.5">
            {!isSingle && !isFirst && (
              <button
                ...
                className="inline-flex items-center gap-0.5 rounded-lg px-2.5 py-1.5 text-[11px] font-medium text-muted-foreground transition-all duration-fast hover:bg-muted dark:text-muted-foreground dark:hover:bg-muted/60"
              >
            ...
            {isLast ? (
              <button
                ...
                className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 py-1.5 text-[11px] font-semibold text-primary-foreground shadow-elevation-1 transition-all duration-base hover:shadow-elevation-2 disabled:cursor-not-allowed disabled:opacity-30 disabled:shadow-none dark:from-blue-500 dark:to-blue-600"
              >
                Submit
              ...
              <button
                type="button"
                onClick={() => setCurrentStep(s => s + 1)}
                className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 py-1.5 text-[11px] font-semibold text-primary-foreground shadow-elevation-1 transition-all duration-base hover:shadow-elevation-2 dark:from-blue-500 dark:to-blue-600"
              >
                Next
                <span className="ml-0.5 font-mono text-[9px] opacity-70">Enter</span>
              </button>
