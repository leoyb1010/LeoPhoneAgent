 ...
 * leocodebox CLI
 *
 * Provides command-line utilities for managing leocodebox
 *
 * Commands:
...
// Match the runtime fallback in load-env.js so "leocodebox status" reports the same default
// database location that the backend will actually use when no DATABASE_PATH is configured.
const DEFAULT_DATABASE_PATH = path.join(os.homedir(), '.leocodebox', 'auth.db');
...
function showStatus(): void {
    ...
    if (hasPendingLegacyDatabaseMigration()) {
        console.log(`       ${c.tip('[TIP]')} Existing legacy data will be copied into this leocodebox path on first server start.`);
    }
    ...
    console.log('\n' + c.dim('═'.repeat(60)));
    console.log(`\n${c.tip('[TIP]')} Hints:`);
    console.log(`      ${c.dim('>')} Use ${c.bright('leocodebox --port 8080')} to run on a custom port`);
    console.log(`      ${c.dim('>')} Use ${c.bright('leocodebox --database-path /path/to/db')} for custom database`);
    console.log(`      ${c.dim('>')} Run ${c.bright('leocodebox help')} for all options`);
...
Examples:
  $ leocodebox                        # Start with defaults
  $ leocodebox --port 8080            # Start on port 8080
  $ leocodebox sandbox ~/my-project   # Run in a Docker sandbox
  $ leocodebox status                 # Show configuration
...
Usage:
  leocodebox sandbox <workspace>            Create and start a sandbox
  leocodebox sandbox <subcommand> [name]    Manage sandboxes
...
Subcommands:
  ...
  ${c.bright('logs')}         Show leocodebox server logs
...
Examples:
  $ leocodebox sandbox ~/my-project
  $ leocodebox sandbox ~/my-project --agent codex --port 8080
  $ leocodebox sandbox ~/my-project --env SERVER_PORT=8080 --env HOST=0.0.0.0
  $ leocodebox sandbox ls
  $ leocodebox sandbox stop my-project
  $ leocodebox sandbox start my-project
  $ leocodebox sandbox rm my-project
...
async function sandboxCommand(args: string[]): Promise<void> {
    ...
    switch (opts.subcommand) {
        ...
        case 'stop':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: leocodebox sandbox stop <name>\n`);
                process.exit(1);
            }
        ...
        case 'rm':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: leocodebox sandbox rm <name>\n`);
                process.exit(1);
            }
        ...
        case 'logs':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: leocodebox sandbox logs <name>\n`);
                process.exit(1);
            }
            try {
        ...
        case 'start': {
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: leocodebox sandbox start <name>\n`);
                process.exit(1);
            }
            ...
            console.log(`\n${c.ok('✔')} ${c.bright('leocodebox is ready!')}`);
        ...
        case 'create': {
            if (!opts.workspace) {
                console.error(`\n${c.error('❌')} Workspace path required: leocodebox sandbox <path>\n`);
                console.log(`   Example: ${c.bright('leocodebox sandbox ~/my-project')}\n`);
                process.exit(1);
            }
            ...
            console.log(`\n${c.dim('  Or install leocodebox from the desktop release package.')}\n`);
            break;
        }
...
async function main(): Promise<void> {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('   Run "leocodebox help" for usage information.\n');
            process.exit(1);
    }
}
