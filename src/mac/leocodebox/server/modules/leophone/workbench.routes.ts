...
router.post('/leophone/local/sessions/:sessionId/log', async (req, res) => {
  const session = requireSession(req, res);
  if (!session) return;
  const hash = String(((req.body ?? {}) as Record<string, unknown>).hash ?? '').trim();
  try {
    res.json(hash ? await showSessionCommit(session.cwd, hash) : await listSessionCommits(session.cwd));
  } catch (error) {
    jsonError(res, 409, error instanceof Error ? error.message : String(error));
  }
});
...
router.post('/leophone/local/sessions/:sessionId/init', async (req, res) => {
  const session = requireSession(req, res);
  if (!session) return;
  try {
...
router.post('/leophone/local/sessions/:sessionId/push', async (req, res) => {
  const session = requireSession(req, res);
  if (!session) return;
  try {
    res.json(await pushSessionRepo(session.cwd));
  } catch (error) {
    jsonError(res, 409, error instanceof Error ? error.message : String(error));
  }
});
...
router.post('/leophone/local/sessions/:sessionId/rewind', async (req, res) => {
  try {
    const result = await getHarnessManager().rewind(req.params.sessionId);
...
/** pi 原生命令(白名单):换模型 / 压缩 / 中止 / 插话 / 排队 / 取消排队 / 思考深度 / 会话 bash / 斜杠 prompt。回执经事件流回来。 */
const RPC_ALLOWED = new Set(['set_model', 'compact', 'abort', 'steer', 'follow_up', 'set_thinking_level', 'clear_queue', 'bash', 'prompt']);
