...
export const LEO_RELEASE_NOTES: LeoRelease[] = [
  ...
  },
  {
    ...
    date: "2026-09-26",
    items: [
      "换上 Leo 自己的样子:暖色中性底 + 与鸿蒙端同一套青绿,圆角、阴影更柔和,欢迎页和引导页也换了配色。打开 App 不再是 ZCode 的样子。",
      "首页只留标志、问候和输入框。有事时输入框下方列出「进行中」:等你确认 / 回答的排最前,然后是在跑的,最后是做完还没看的(最多 4 条,点一下直接打开);没事就不出现。",
      "动效更安静也更连贯:聊天 / 自动化 / 插件 / 设置之间切换会淡入,审批卡从输入框的位置滑入,任务在跑时输入框顶边有一道细细的青色光带;系统开了「减少动态效果」就都不动。",
      "左下角手机图标 →「移动端远程控制」最上面能看到这台 Mac 连没连上中继。中继升到 0.2 后,在这里点「扫码连接手机」给新手机出码,手机上 设置 → 远程机器 → 扫码添加机器:每台手机领自己的钥匙,不用再抄,丢一台不影响其他;码 5 分钟过期、只能用一次。现在连着的中继还是旧版,按钮会显示「中继待升级」。",
      "改代码更稳:一次 Edit 可以同时改同一文件的多处(都按原文件定位,要么一起成功要么都不改);模型传错参数会自动修正;弯引号、全角空格、行尾空白这类差异也能对上原文;文件的 BOM 和 CRLF 行尾保持不变;同一文件的多次改写排队执行,不再互相覆盖。",
      "上下文快满、回答被截断时,先压缩一次再重试这一步,半截回答不再混进后面的请求;工具列表顺序固定,提示缓存更容易命中;DeepSeek、Kimi 的缓存命中数也记进用量了。",
      "新增几个可选档位,默认都关,在 ~/.leophoneagent/cli/config.json 的 \"leo\" 段打开:GLM / Kimi / MiniMax 用带哈希锚点的编辑格式、精简模式(短提示词 + 核心工具)、读文件不带行号。",
    ],
  },
  {
    version: "1.2.5",
